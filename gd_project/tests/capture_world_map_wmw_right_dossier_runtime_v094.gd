...
const FIXTURE_PATH := "res://Assets/ui/angus_packaging/world_map/wmw_v094_right_dossier_candidate_a4/right_dossier_candidate_a4_runtime_fixture.json"
...
const INLINE_DOCUMENT_RECT := Rect2(24.0, 14.0, 16.0, 16.0)
const INLINE_LABEL_RECT := Rect2(58.0, 12.0, 56.0, 20.0)
const INLINE_CHEVRON_RECT := Rect2(112.0, 18.0, 10.0, 8.0)
const INLINE_DIVIDER_RECT := Rect2(16.0, 42.0, 252.0, 1.0)
...
func _run() -> void:
	...
	if _primary_texture.get_size() != Vector2(568, 100):
		push_error("A4 reused primary action child dimensions drifted")
		quit(1)
		return
	if not _validate_photo_contract():
		...
		return
	var ok := true
	ok = await _capture(false, "615-world-map-wmw-v0-9-4-right-dossier-candidate-a4-godot-runtime.png") and ok
	ok = await _capture(true, "616-world-map-wmw-v0-9-4-right-dossier-candidate-a4-godot-runtime-qa.png") and ok
	if not ok:
		push_error("Godot v0.9.4 right-dossier candidate A4 capture failed.")
		quit(1)
		return
	print("capture_world_map_wmw_right_dossier_runtime_v094.gd OK")
	quit(0)
...
func _load_runtime_fixture() -> bool:
	...
	if str(_runtime_fixture["status_display_text"]).contains("\n"):
		push_error("A4 status must remain a single semantic line")
		return false
	if str(_runtime_fixture["mission_intel_label"]) != "任务情报" or str(_runtime_fixture["mission_intel_expanded_label"]) != "任务情报":
		push_error("A209 disclosure title drifted")
		return false
	if str(_runtime_fixture["mission_intel_label"]).contains("项"):
		push_error("Mission count leaked into A4 disclosure title")
		return false
...
func _build_scene() -> void:
	...
	_add_label("Godot 4.6.2 · WMW right dossier candidate A4", Rect2(660, 56, 1160, 48), 31, Color("#f2ead0"), HORIZONTAL_ALIGNMENT_LEFT)
	_add_label("A209: inline disclosure header vs sole entity CTA; two hit rects, one visual button.", Rect2(662, 106, 1120, 34), 18, Color("#c8d7c8"), HORIZONTAL_ALIGNMENT_LEFT)
	...
	_add_label("Disclosure: transparent paper, regular title and small chevron. Navigation: the only filled entity button.", Rect2(710, 625, 930, 70), 19, Color("#dfe2c8"), HORIZONTAL_ALIGNMENT_LEFT)
...
func _add_dossier() -> void:
	...
	_add_icon(_document_icon, _child_slot_rect(mission_rect, INLINE_DOCUMENT_RECT), Color("#3f6663"), 1.0)
	_add_label(str(_runtime_fixture["mission_intel_label"]), _child_slot_rect(mission_rect, INLINE_LABEL_RECT), 18, Color("#3f6663"), HORIZONTAL_ALIGNMENT_LEFT)
	_add_chevron(_child_slot_rect(mission_rect, INLINE_CHEVRON_RECT), Color("#3f6663"))
	_add_divider(_child_slot_rect(mission_rect, INLINE_DIVIDER_RECT), Color(0.247, 0.4, 0.388, 0.28))
...
func _add_chevron(rect: Rect2, color: Color) -> void:
	var chevron := Line2D.new()
	chevron.default_color = Color(color.r, color.g, color.b, 0.9)
	chevron.width = 2.25
	chevron.antialiased = true
	chevron.add_point(rect.position + Vector2(rect.size.x * 0.12, rect.size.y * 0.28))
	chevron.add_point(rect.position + Vector2(rect.size.x * 0.5, rect.size.y * 0.72))
	chevron.add_point(rect.position + Vector2(rect.size.x * 0.88, rect.size.y * 0.28))
...
func _add_qa_overlays() -> void:
	...
	var mission_rect := _slot_rect("mission_intel_button")
	var primary_rect := _slot_rect("primary_enter_cta")
	_add_rect(_child_slot_rect(mission_rect, INLINE_DOCUMENT_RECT), Color("#58edf4"), "inline_doc")
	_add_rect(_child_slot_rect(mission_rect, INLINE_LABEL_RECT), Color("#ff48ca"), "inline_title")
	_add_rect(_child_slot_rect(mission_rect, INLINE_CHEVRON_RECT), Color("#58edf4"), "inline_chevron")
	_add_rect(_child_slot_rect(mission_rect, INLINE_DIVIDER_RECT), Color("#58edf4"), "divider")
	_add_rect(_child_slot_rect(mission_rect, MISSION_SLOTS["right_action_badge"]), Color("#82908e"), "retired_badge_zero_ink")
