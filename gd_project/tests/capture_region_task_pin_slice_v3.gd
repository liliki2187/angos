...
const OUT_DIR := "res://../docs/screenshots/2026-07-17-region-task-pin-slice-v3"
...
func _run() -> void:
	...
	await _capture_board(_fixture_payload(5, "event_2", true), "01-five-dense-selected.png")
	...
	print("capture_region_task_pin_slice_v3.gd OK")
	quit(0)
...
func _capture_state_matrix() -> void:
	...
	_add_label(stage, "区域任务图钉 C 混合竖切片 · 真实 Godot 运行态", Rect2(74, 48, 1500, 52), 30, Color("f2ead6"))
	_add_label(stage, "C 切角主形 + B 安静内容面 + A 单侧脊线；图标、选中背板、局部弧和状态签分层运行", Rect2(76, 104, 1660, 38), 17, Color("aebc9a"))
...
func _fixture_payload(event_count: int, selected_id: String, force_dense: bool) -> Dictionary:
	...
	for index in range(event_count):
		...
		nodes.append({
			...
			"enabled": false if index == 4 else true,
	...
	return {
		...
		"region_node_deadline": "截稿前 2 天" if not selected_id.is_empty() else "",
