...
const FIXTURE_PATH := "res://Assets/ui/angus_packaging/world_map/wmw_v092_right_dossier_candidate_a2/right_dossier_candidate_a2_runtime_fixture.json"
...
func _run() -> void:
	...
	if _parent_texture == null or _mission_texture == null or _primary_texture == null or _photo_texture == null or _globe_icon == null or _document_icon == null or _arrow_icon == null:
		quit(1)
		return
	if not _validate_photo_contract():
		...
		return
	var ok := true
	ok = await _capture(false, "601-world-map-wmw-v0-9-2-right-dossier-candidate-a2-godot-runtime.png") and ok
	ok = await _capture(true, "602-world-map-wmw-v0-9-2-right-dossier-candidate-a2-godot-runtime-qa.png") and ok
	if not ok:
		push_error("Godot v0.9.2 right-dossier candidate A2 capture failed.")
		quit(1)
		return
	print("capture_world_map_wmw_right_dossier_runtime_v092.gd OK")
	quit(0)
...
func _load_runtime_fixture() -> bool:
	...
	if str(_runtime_fixture["status_display_text"]).contains("\n"):
		push_error("A2 status must remain a single semantic line")
		return false
...
func _build_scene() -> void:
	...
	_add_label("Godot 4.6.2 · WMW right dossier candidate A2", Rect2(660, 56, 1160, 48), 31, Color("#f2ead0"), HORIZONTAL_ALIGNMENT_LEFT)
	_add_label("A205: production region payload only · recommendation retired · object + direction icon grammar.", Rect2(662, 106, 1120, 34), 18, Color("#c8d7c8"), HORIZONTAL_ALIGNMENT_LEFT)
	...
	_add_label("Mission: document + arrow. Enter: globe + arrow. Each row remains one command and one hit target.", Rect2(710, 625, 930, 70), 19, Color("#dfe2c8"), HORIZONTAL_ALIGNMENT_LEFT)
	_add_label("A1 shell, exact 69:44 photo, optical axis and safe inset are reused without changing frozen geometry.", Rect2(710, 755, 930, 70), 18, Color("#c8d7c8"), HORIZONTAL_ALIGNMENT_LEFT)
...
func _add_dossier() -> void:
	...
	_add_label(str(_runtime_fixture["mission_intel_label"]), mission_label, 18, Color("#1f5255"), HORIZONTAL_ALIGNMENT_CENTER)
	...
	_add_icon(_globe_icon, primary_left, Color("#eee5c9"), 0.72)
	_add_label(str(_runtime_fixture["primary_enter_label"]), primary_label, 20, Color("#3a4229"), HORIZONTAL_ALIGNMENT_CENTER)
	_add_icon(_arrow_icon, primary_right, Color("#eee5c9"), 0.78)
