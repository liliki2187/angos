...
const OUT_PATH := "res://Assets/ui/angus_packaging/world_map/wmw_v093_right_dossier_candidate_a3/right_dossier_candidate_a3_runtime_fixture.json"
...
func _export_fixture() -> void:
	...
	var fixture := {
		"schema_version": 1,
		"candidate": "right_dossier_candidate_a3_v0.9.3",
		...
		"decision_facts": "任务构成 · 限时 %d · 线索 %d · 深链 %d" % [
			int(counts["deadline"]),
			int(counts["clue"]),
			int(counts["chain"]),
		],
		"mission_intel_label": str(Systems.Content.WORLD_MAP_UI_COPY["mission_intel_collapsed"]),
		"mission_intel_expanded_label": str(Systems.Content.WORLD_MAP_UI_COPY["mission_intel_expanded"]),
