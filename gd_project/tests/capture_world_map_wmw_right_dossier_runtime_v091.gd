...
const MISSION_PATH := "res://Assets/ui/angus_packaging/world_map/wmw_v091_right_dossier_candidate_a1/ingredients/right_mission_intel_button_candidate_a1_teal_2x.png"
const PRIMARY_PATH := "res://Assets/ui/angus_packaging/world_map/wmw_v091_right_dossier_candidate_a1/ingredients/right_action_lane_candidate_a1_primary_olive_2x.png"
...
const CHECK_ICON_PATH := "res://Assets/ui/angus_packaging/world_map/wmw_v091_right_dossier_candidate_a1/ingredients/runtime_icons/right_dossier_runtime_check.png"
...
func _run() -> void:
	...
	if not _validate_photo_contract():
		...
		return
	var ok := true
	ok = await _capture(false, "594-world-map-wmw-v0-9-1-right-dossier-candidate-a1-godot-runtime.png") and ok
	ok = await _capture(true, "595-world-map-wmw-v0-9-1-right-dossier-candidate-a1-godot-runtime-qa.png") and ok
	if not ok:
		push_error("Godot v0.9.1 right-dossier candidate A1 capture failed.")
		quit(1)
		return
	print("capture_world_map_wmw_right_dossier_runtime_v091.gd OK")
	quit(0)
...
func _build_scene() -> void:
	...
	_add_label("Godot 4.6.2 · WMW right dossier candidate A1", Rect2(660, 56, 1160, 48), 31, Color("#f2ead0"), HORIZONTAL_ALIGNMENT_LEFT)
	_add_label("A201 runtime correction: ink optical axis + body safe inset + exact 69:44 photo.", Rect2(662, 106, 1120, 34), 18, Color("#c8d7c8"), HORIZONTAL_ALIGNMENT_LEFT)
	_add_label("HEADER OPTICAL AXIS", Rect2(710, 230, 360, 34), 21, Color("#65ff94"), HORIZONTAL_ALIGNMENT_LEFT)
	_add_label("Icon +3px · title 0px · status group -4px. Offsets apply to actual content, not frozen carriers.", Rect2(710, 270, 940, 70), 19, Color("#dfe2c8"), HORIZONTAL_ALIGNMENT_LEFT)
	_add_label("BODY INNER", Rect2(710, 390, 220, 34), 21, Color("#ffd444"), HORIZONTAL_ALIGNMENT_LEFT)
	_add_label("Outer slot stays frozen. Text uses [34,290,252,50]; the cyan index strip is a hard no-text region.", Rect2(710, 430, 940, 86), 19, Color("#dfe2c8"), HORIZONTAL_ALIGNMENT_LEFT)
	_add_label("PHOTO CONTRACT", Rect2(710, 585, 250, 34), 20, Color("#48ecae"), HORIZONTAL_ALIGNMENT_LEFT)
	_add_label("1104×704 source → 552×352 ingredient → 414×264 runtime; uniform scale only.", Rect2(710, 625, 930, 70), 19, Color("#dfe2c8"), HORIZONTAL_ALIGNMENT_LEFT)
	_add_label("TextureRect uses KEEP_ASPECT_COVERED. STRETCH_SCALE is forbidden for photo content.", Rect2(710, 755, 930, 70), 18, Color("#c8d7c8"), HORIZONTAL_ALIGNMENT_LEFT)
...
func _add_dossier() -> void:
	...
	_add_label("天线阵列仍在发射，林线内出现异常回波。\n进入前可先核对本周地区任务与封锁条件。", _reference_rect(REGION_BODY_INNER), 16, Color("#22251f"), HORIZONTAL_ALIGNMENT_LEFT)
