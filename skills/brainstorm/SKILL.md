...
- Treat references like /brainstorm or /other-skill as Codex skill names. For example, use $brainstorm for this workflow.
...
3. **Run through ideation phases** interactively, asking the user questions at
   ...
   **Use `a concise direct user question`** at key decision points throughout brainstorming:
   ...
   `a concise direct user question` to capture the decision with concise labels.
...
4. **Generate the game concept document** using the template at
   `../_game-studio-shared/templates/game-concept.md`. Fill in ALL sections from the
