#!/usr/bin/env python3
"""Validate the Angus Game System Architect skill without third-party dependencies."""
...
REQUIRED_FILES = [
    "skills/game-sys-architect/SKILL.md",
    "skills/game-sys-architect/agents/openai.yaml",
    "skills/game-sys-architect/references/system-prompt-v1.0.md",
    "skills/game-sys-architect/references/upstream-readme.md",
    "skills/game-sys-architect/references/casebook/case-01-weekly-architecture.md",
    ".codex/agents/game-sys-architect.toml",
]

DOC_REQUIRED_PHRASES_BY_FILE = {
    "AGENTS.md": ["game_sys_architect", "@系统架构", "map-systems"],
    "docs/onboarding/ai-collaboration-guidance.md": ["game_sys_architect", "@系统架构", "map-systems"],
    "docs/onboarding/subagent-collaboration-improvement.md": ["game_sys_architect", "@系统架构", "map-systems"],
}
...
class Validator:
    ...
    def validate_skill_md(self) -> None:
        rel = "skills/game-sys-architect/SKILL.md"
        ...
        if frontmatter is None:
            return
        if frontmatter.get("name") != "game-sys-architect":
            self.fail("SKILL.md frontmatter name must be game-sys-architect")
    ...
    def load_toml(self, rel: str) -> dict[str, object] | None:
        ...
        if "developer_instructions" not in data and 'developer_instructions = """' in text:
            data["developer_instructions"] = text
        return data

    def validate_agent_toml(self) -> None:
        rel = ".codex/agents/game-sys-architect.toml"
        data = self.load_toml(rel)
        if data is None:
            return
        expected = {
            "name": "game_sys_architect",
        ...
        for phrase in [
            "skills/game-sys-architect/SKILL.md",
            "read-only gameplay system architecture",
            "Do not write code",
            "Do not replace map-systems",
        ]:
            if phrase not in instructions:
                self.fail(f"{rel} developer_instructions missing: {phrase}")
        self.ok("game-sys-architect.toml basic fields parse")

    def validate_openai_yaml(self) -> None:
        rel = "skills/game-sys-architect/agents/openai.yaml"
        text = self.read_text(rel)
        if text is None:
            return
        for phrase in ["display_name", "short_description", "default_prompt", "Game System Architect"]:
            if phrase not in text:
    ...
    def validate_project_docs(self) -> None:
        ...
        self.ok("project routing docs contain expected System Architect routing references")
...
def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(description="Validate Angus Game System Architect skill without dependencies.")
