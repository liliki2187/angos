...
- Treat references like /design-system or /other-skill as Codex skill names. For example, use $design-system for this workflow.
...
Use `a concise direct user question`:
...
Use the template structure from `../_game-studio-shared/templates/game-design-document.md`:
...
2. **Questions**: Ask clarifying questions specific to this section. Use
   `a concise direct user question` for constrained questions, conversational text for open-ended
   exploration.
...
3. **Options**: Where the section involves design choices (not just documentation),
   ...
   then use `a concise direct user question` to capture the decision.
...
**Agent delegation**: For complex mechanics, delegate a bounded subtask to a suitable generic Codex subagent for
...
delegate to `systems-designer` via a suitable generic Codex subagent. Provide:
...
**When delegating with generic Codex subagents**:
...
- The main session presents the agent's output to the user via `a concise direct user question`
...
2. **a concise direct user question** at every decision point (Explain -> Capture pattern):
