...
$DefaultCtiHome = Join-Path $SkillsDir '.claude-to-im'
...
function Start-Fallback {
    ...
    $hadDuplicatePath = -not [string]::IsNullOrEmpty($originalPath) -and -not [string]::IsNullOrEmpty($originalUpperPath)

    try {
        # PowerShell on Windows can carry both Path and PATH in the process env.
        # Start-Process treats keys case-insensitively and throws if both exist.
        if ($hadDuplicatePath) {
...
switch ($Command) {
    ...
    'restart' {
        ...
        if ($newPid -and (Test-PidAlive $newPid) -and (Test-StatusRunning)) {
            ...
            Write-Host "Failed to restart bridge."
            Show-LastExitReason
            Show-FailureHelp
            exit 1
        }
    ...
    'help' {
        ...
        Write-Host "  restart           Restart the bridge daemon and verify it comes back"
