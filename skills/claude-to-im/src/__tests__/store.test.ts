...
describe('JsonFileStore', () => {
  ...
  it('clearSessionContext removes stored messages and persisted attachment files', () => {
    const store = new JsonFileStore(makeSettings());
    const session = store.createSession('test', 'model', undefined, '/tmp');
    const uploadDir = path.join(DATA_DIR, 'uploads');
    fs.mkdirSync(uploadDir, { recursive: true });
    const filePath = path.join(uploadDir, 'diagram.png');
    fs.writeFileSync(filePath, 'demo');

    store.addMessage(
      session.id,
      'user',
      `<!--files:${JSON.stringify([{ filePath, name: 'diagram.png', type: 'image/png', size: 4 }])}-->hello`,
    );
    store.addMessage(session.id, 'assistant', 'world');

    const result = store.clearSessionContext(session.id);

    assert.deepEqual(result, { removedMessages: 2, removedFiles: 1 });
    assert.deepEqual(store.getMessages(session.id), { messages: [] });
    assert.equal(fs.existsSync(path.join(DATA_DIR, 'messages', `${session.id}.json`)), false);
