...
describe('CodexProvider', () => {
  ...
  it('replays stored history when resumeThread throws synchronously', async () => {
    const { CodexProvider } = await import('../codex-provider.js');
    const { PendingPermissions } = await import('../permission-gateway.js');
    const provider = new CodexProvider(new PendingPermissions());
    ...
    const stream = provider.streamChat({
      prompt: '继续聊',
      sessionId: 'resume-sync-throw',
      sdkSessionId: 'dead-thread',
      conversationHistory: [{ role: 'user', content: '上一轮提到过 clip.mp4' }],
    });

    await collectStream(stream);

    assert.equal(startCalls, 1, 'Should recover with a fresh thread');
    assert.equal(typeof capturedInput, 'string');
    assert.ok((capturedInput as string).includes('Conversation history from the same IM chat:'));
    assert.ok((capturedInput as string).includes('User: 上一轮提到过 clip.mp4'));
  });
});

describe('Codex prompt helpers', () => {
  it('builds prompt text with history and non-image attachment summary', () => {
    const prompt = buildPromptText(
      '帮我看看',
      [{ id: 'f1', name: 'voice.mp3', type: 'audio/mpeg', size: 2048, data: 'abcd' }],
      [{ role: 'assistant', content: '之前已经看过一个版本。' }],
    );

    assert.ok(prompt.includes('Conversation history from the same IM chat:'));
    assert.ok(prompt.includes('Assistant: 之前已经看过一个版本。'));
    assert.ok(prompt.includes('Attached non-image files:'));
    assert.ok(prompt.includes('- voice.mp3 (audio/mpeg, 2.0 KB)'));
  });
});
...
describe('Codex thread options', () => {
  it('defaults bridge-launched Codex threads to danger-full-access', () => {
    ...
    try {
      ...
      if (oldNetwork === undefined) delete process.env.CTI_CODEX_NETWORK_ACCESS;
      else process.env.CTI_CODEX_NETWORK_ACCESS = oldNetwork;
...
describe('CodexProvider image input', () => {
  ...
  it('builds local_image input with multiple images while summarizing non-image files', async () => {
    ...
    assert.equal(parts[0].type, 'text');
    assert.ok(parts[0].text.includes('Attached non-image files:'));
    assert.ok(parts[0].text.includes('- c.txt (text/plain, 8 B)'));
    assert.equal(parts[1].type, 'local_image');
  ...
  it('passes non-image attachment metadata when no images are attached', async () => {
    const { CodexProvider } = await import('../codex-provider.js');
    const { PendingPermissions } = await import('../permission-gateway.js');
    const provider = new CodexProvider(new PendingPermissions());
    ...
    const stream = provider.streamChat({
      ...
      files: [makeFile('video/mp4', 'abcd', 'clip.mp4')],
    });

    await collectStream(stream);

    assert.equal(typeof capturedInput, 'string');
    assert.ok((capturedInput as string).includes('Attached non-image files:'));
    assert.ok((capturedInput as string).includes('- clip.mp4 (video/mp4, 4 B)'));
  });
});
