...
describe('history + attachment prompt helpers', () => {
  it('normalizes stored file metadata comments into readable text', () => {
    const normalized = normalizeStoredMessageContent(
      '<!--files:[{"name":"clip.mp4","type":"video/mp4","size":2048}]-->hello world',
    );

    assert.ok(normalized.includes('Attached files: clip.mp4 (video/mp4, 2.0 KB)'));
  ...
  it('builds history prelude with normalized user and assistant messages', () => {
    const prelude = buildHistoryPrelude([
      {
        role: 'user',
        content: '<!--files:[{"name":"voice.mp3","type":"audio/mpeg","size":1024}]-->请看这个文件',
      },
      {
        role: 'assistant',
        content: JSON.stringify([{ type: 'text', text: '我看到了附件。' }]),
      },
    ]);

    assert.ok(prelude.includes('Conversation history from the same IM chat:'));
    assert.ok(prelude.includes('User: Attached files: voice.mp3 (audio/mpeg, 1.0 KB)'));
    assert.ok(prelude.includes('Assistant: 我看到了附件。'));
  });

  it('builds plain-text prompt with history and non-image attachment summary', () => {
    const prompt = buildPrompt(
      '帮我总结',
      [makeFile('video/mp4', 'abcd', 'clip.mp4')],
      [{ role: 'user', content: '上一轮问题' }],
    );

    assert.equal(typeof prompt, 'string');
    assert.ok((prompt as string).includes('Conversation history from the same IM chat:'));
    assert.ok((prompt as string).includes('User: 上一轮问题'));
    assert.ok((prompt as string).includes('Attached non-image files:'));
    assert.ok((prompt as string).includes('- clip.mp4 (video/mp4, 4 B)'));
  });

  it('builds multimodal prompt with image plus non-image file summary', async () => {
    const prompt = buildPrompt(
      '',
      [
        makeFile('image/png', 'aGVsbG8=', 'image.png'),
        makeFile('application/pdf', 'abcd', 'brief.pdf'),
      ],
      [{ role: 'assistant', content: '上次我们讨论过这份文件。' }],
    );

    assert.notEqual(typeof prompt, 'string');
    const iterator = prompt as AsyncIterable<{
    ...
    assert.ok(content[1].text?.includes('Assistant: 上次我们讨论过这份文件。'));
    assert.ok(content[1].text?.includes('Attached non-image files:'));
    assert.ok(content[1].text?.includes('- brief.pdf (application/pdf, 4 B)'));
  });
});
