...
function extractAttachmentFilePaths(content: string): string[] {
  const match = content.match(/^<!--files:(.*?)-->/s);
  if (!match) return [];
  try {
    const parsed = JSON.parse(match[1]) as Array<{ filePath?: string }>;
...
export class JsonFileStore implements BridgeStore {
  ...
  clearSessionContext(sessionId: string): { removedMessages: number; removedFiles: number } {
    const msgs = this.loadMessages(sessionId);
    const filePaths = new Set<string>();
    for (const msg of msgs) {
      for (const filePath of extractAttachmentFilePaths(msg.content)) {
