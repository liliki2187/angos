...
// Override the upstream Feishu adapter with the workspace-local patched version.
...
async function sendFeishuStartupNotice(config: Config, store: JsonFileStore, _runId: string): Promise<void> {
  if (!config.enabledChannels.includes('feishu')) return;
  if (!config.feishuAppId || !config.feishuAppSecret) return;

  const targetBinding = store
    .listChannelBindings('feishu')
    .filter((binding) => binding.active)
    .sort((a, b) => Date.parse(b.updatedAt || b.createdAt) - Date.parse(a.updatedAt || a.createdAt))[0];
  const targetChatId = config.feishuDefaultChatId || targetBinding?.chatId;
  if (!targetChatId) {
    console.log('[claude-to-im] Startup notice skipped: no active Feishu chat binding');
    return;
  }

  try {
    const lark = await import('@larksuiteoapi/node-sdk');
    const domain = (config.feishuDomain || '').includes('larksuite')
      ? lark.Domain.Lark
      : lark.Domain.Feishu;
    ...
    if (response?.data?.message_id) {
      console.log(
        `[claude-to-im] Startup notice sent to ${targetChatId} (${response.data.message_id})`,
      );
      return;
    }

    console.warn(
      `[claude-to-im] Startup notice returned no message_id for ${targetChatId}`,
    );
  } catch (err) {
    console.warn(
      '[claude-to-im] Startup notice failed:',
...
async function main(): Promise<void> {
  ...
  await sendFeishuStartupNotice(config, store, runId);
