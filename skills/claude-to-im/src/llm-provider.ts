...
function summarizeFiles(files?: FileAttachment[]): string[] {
  if (!files || files.length === 0) return [];
  return files.map((file) => `- ${file.name} (${file.type}, ${formatAttachmentSize(file.size)})`);
}
...
export function buildPrompt(
  ...
  const attachmentSummary = summarizeFiles(nonImageFiles);

  let effectiveText = text.trim();
  if (!effectiveText && files && files.length > 0) {
    effectiveText = imageFiles.length > 0 && nonImageFiles.length === 0
    ...
    return new ReadableStream({
      start(controller) {
        (async () => {
          ...
          let retryFreshSession = false;
          let resumeSessionId = params.sdkSessionId || undefined;
          ...
          try {
            ...
            console.warn(
              '[llm-provider] Feishu history backfill failed, continuing without it:',
          ...
          while (true) {
            ...
            try {
              ...
              if (
                ...
                !retryFreshSession &&
                !sawAnyMessage &&
                !state.hasReceivedResult &&
                !state.hasStreamedText &&
                shouldRetryFreshSession(`${message}\n${stderrBuf}`)
              ) {
                console.warn("[llm-provider] Resume failed before any output, retrying with fresh session and stored history");
