...
- Treat references like /map-systems or /other-skill as Codex skill names. For example, use $map-systems for this workflow.
...
- Use `a concise direct user question` to ask:
...
Then use `a concise direct user question` to capture feedback:
...
Use `a concise direct user question` to ask: "Does this dependency ordering look right? Any
...
Use `a concise direct user question` to ask: "Do these priority assignments match your vision?
...
Using the template at `../_game-studio-shared/templates/systems-index.md`, populate the
...
Use `a concise direct user question` for: "Start designing [system-name] now, pick a different
system, or stop here?"
...
After `$design-system` completes, use `a concise direct user question`:
