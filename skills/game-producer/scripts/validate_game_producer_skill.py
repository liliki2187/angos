#!/usr/bin/env python3
"""Validate the Angus Game Producer skill without third-party dependencies."""
...
REQUIRED_FILES = [
    "skills/game-producer/SKILL.md",
    "skills/game-producer/agents/openai.yaml",
    "skills/game-producer/references/system-prompt-v1.1.md",
    "skills/game-producer/references/upstream-readme.md",
    "skills/game-producer/references/casebook/case-01-greenlight.md",
    ".codex/agents/game-producer.toml",
]

DOC_REQUIRED_PHRASES_BY_FILE = {
    "AGENTS.md": ["game_producer", "@制作人", "steam_indie_appraiser", "不插入每次 UI"],
    "docs/onboarding/ai-collaboration-guidance.md": ["game_producer", "@制作人", "steam_indie_appraiser"],
    "docs/onboarding/subagent-collaboration-improvement.md": ["game_producer", "@制作人", "steam_indie_appraiser"],
}

SKILL_REQUIRED_PHRASES = [
    "@制作人",
    "不覆盖 `steam_indie_appraiser`",
    "不插入每次 UI",
...
class Validator:
    ...
    def validate_skill_md(self) -> None:
        rel = "skills/game-producer/SKILL.md"
        ...
        if frontmatter is None:
            return
        if frontmatter.get("name") != "game-producer":
            self.fail("SKILL.md frontmatter name must be game-producer")
    ...
    def validate_agent_toml(self) -> None:
        rel = ".codex/agents/game-producer.toml"
        ...
        for phrase in [
            "skills/game-producer/SKILL.md",
            "Do not replace steam_indie_appraiser",
            "Do not join every UI design task",
        ]:
            if phrase not in instructions:
                self.fail(f"{rel} developer_instructions missing: {phrase}")
        self.ok("game-producer.toml basic fields parse")

    def validate_openai_yaml(self) -> None:
        rel = "skills/game-producer/agents/openai.yaml"
        text = self.read_text(rel)
        if text is None:
            return
        for phrase in ["display_name", "short_description", "default_prompt", "Game Producer"]:
            if phrase not in text:
    ...
    def validate_project_docs(self) -> None:
        ...
        self.ok("project routing docs contain expected Producer/SIA routing references")
...
def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(description="Validate Angus Game Producer skill without dependencies.")
