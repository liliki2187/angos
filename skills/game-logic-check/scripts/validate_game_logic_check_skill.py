#!/usr/bin/env python3
"""Validate the Angus Game Logic Check skill without third-party dependencies."""
...
REQUIRED_FILES = [
    "skills/game-logic-check/SKILL.md",
    "skills/game-logic-check/agents/openai.yaml",
    "skills/game-logic-check/references/system-prompt-v1.0.md",
    "skills/game-logic-check/references/upstream-readme.md",
    "skills/game-logic-check/references/casebook/case-01-faction-system.md",
    ".codex/agents/game-logic-check.toml",
]

DOC_REQUIRED_PHRASES_BY_FILE = {
    "AGENTS.md": ["game_logic_check", "@逻辑审查", "只读"],
    "docs/onboarding/ai-collaboration-guidance.md": ["game_logic_check", "@逻辑审查", "只读"],
    "docs/onboarding/subagent-collaboration-improvement.md": ["game_logic_check", "@逻辑审查", "只读"],
}
...
class Validator:
    ...
    def validate_skill_md(self) -> None:
        rel = "skills/game-logic-check/SKILL.md"
        ...
        if frontmatter is None:
            return
        if frontmatter.get("name") != "game-logic-check":
            self.fail("SKILL.md frontmatter name must be game-logic-check")
    ...
    def validate_agent_toml(self) -> None:
        rel = ".codex/agents/game-logic-check.toml"
        data = self.load_toml(rel)
        if data is None:
            return
        expected = {
            "name": "game_logic_check",
        ...
        for phrase in [
            ...
            "Do not run during early brainstorming",
        ]:
            if phrase not in instructions:
                self.fail(f"{rel} developer_instructions missing: {phrase}")
        self.ok("game-logic-check.toml basic fields parse")

    def validate_openai_yaml(self) -> None:
        rel = "skills/game-logic-check/agents/openai.yaml"
        text = self.read_text(rel)
        if text is None:
            return
        for phrase in ["display_name", "short_description", "default_prompt", "Game Logic Check"]:
            if phrase not in text:
    ...
    def validate_project_docs(self) -> None:
        ...
        self.ok("project routing docs contain expected Logic Check routing references")
...
def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(description="Validate Angus Game Logic Check skill without dependencies.")
