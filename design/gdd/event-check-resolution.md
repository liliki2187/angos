...
普通任务继续使用基础结果层级。标记为高危的任务在结算展示与奖励映射上扩展为五档：

| 五档结果 | 含义 |
|----------|------|
| 大成功 | 高收益且代价低。 |
| 成功但反噬 | 拿到强素材，但同步触发公信、狂性或其他宏观代价。 |
| 部分成功 | 能推进调查，但材料不完整。 |
| 失败但有线索 | 任务失败，但留下弱线索或下一阶段可用信息。 |
| 大失败 | 明显负面或断线，可能触发长期压力。 |

首轮 HTML 原型中，高危五档优先用于北美禁区带的红色临时情报任务与深度调查连续任务后段。连续任务最终阶段在中间档结果后可插入「再追一次？」临时事件：玩家可收手，也可消耗 1 行动点加入更糟黑骰进行 bonus 判定。该 bonus 的长期风险继续走 `macro`、周事件与员工影响，不建立独立混沌袋。
...
| 高危五档结果说明 | 结果区 | 高危任务结算时 | `riskTier == high` |
| 再追一次事件 | 结果区临时事件卡 | 连续任务终局中间档后 | 任务允许 bonus 且仍有行动点 |
