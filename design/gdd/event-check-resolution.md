...
普通任务继续使用基础结果层级。当前已明确区分 **高危任务** 与 **黑骰任务**：

- **高危任务**：通常直接出现在地图上，或由任务链生成。它只是难度更高、失败后果更重，可能带来 debuff，但机制仍应简单，不使用黑骰。
...
后续进入角色骰系统后，普通任务暂只看是否达标；黑骰任务才使用上述四档。旧 HTML 原型中的“高危五档”表述应视为历史过渡方案。
...
| 高危任务风险说明 | 任务面板 / 结果区 | 高危任务配置与结算时 | `riskTier == high`，但无黑骰 |
