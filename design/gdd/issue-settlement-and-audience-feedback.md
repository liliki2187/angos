# 发刊结算与受众反馈

> **状态**：草案
> **作者**：Codex + 策划稿整理
> **最后更新**：2026-06-02
> **对应支柱**：传播必须推动现实偏移

## 概览

这个系统把已经排好的周刊转成世界后果。它负责销量、营收、受众反馈、势力反应、宏观属性变化和下一周钩子的结算。它只消费“这一期最终发了什么”，不回头改写稿件结构，同时正式接收来自排版系统的 `公开取向`、`同题疲劳` 与 `取向冲突惩罚` 结果。

## 玩家幻想

玩家应感受到自己真的把一期危险周刊送进了世界。发刊不是按一下结算按钮，而是看着本周的选择如何同时改变读者、势力、周刊身份和现实。

## 详细设计

### 核心规则

1. 结算输入是最终版面结构、刊登稿件属性、本周任务结果和当前长期状态。
2. 结算至少输出销量、营收、势力反应、宏观属性变化和下一周状态钩子。
3. 受众反馈必须可读。MVP 可先聚焦少量核心读者群，而不追求完整市场模拟。
4. 发刊结算不负责重新生成稿件，只对最终刊印结果做后果计算。
5. 势力任务完成与否必须在这里产生明确奖励或报复。
6. `抢先快讯` 不参与公开取向历史；其他公开稿件若与同题既往取向冲突，结算必须应用需求惩罚。
7. 同题疲劳应在进入结算前已经折算为稿件有效基础值，结算只消费折算结果，不反向改写稿件。
8. 现实偏移反馈应在结算后显式展示，而不是埋在隐藏变量里。
9. 本系统是 `weekly-run-loop.summary` 父阶段的子状态机；它写回 `settlement_result` 与 `next_week_hooks`，但不跳过结果展示直接开启新周。
10. 当前阶段的读者来信只作为发刊后的薄层反馈牌使用，不新增独立收件箱、逐封回信、读者好感度、多读者群模拟或长期读者关系系统。

### 发刊回响薄层

当前 MVP / 垂直切片优先把“读者来信”处理为发刊结算后的少量反馈摘录，而不是完整读者系统。它服务三件事：让本期发刊产物可截图、提示玩家公开解释被如何理解、把下一周钩子带回 `briefing`。

每期发刊后默认展示 2-3 张反馈牌：

| 反馈牌 | 来源绑定 | 作用 |
|------|---------|------|
| 读者摘录 | 本期最高传播稿件、最强公开取向或最冲突报道 | 用一句黑色幽默反馈说明公众如何误读、放大或认领本期解释 |
| 势力 / 广告 / 机构反应 | 本期势力任务、违规点或高价值版面 | 给出压力、禁令、合作、威胁或赞助变化 |
| 现实回响 | 最异常报道、最强路线倾向或本期冲突点 | 生成地图异样、新线索、任务变体、地区异动或下周开局事件钩子 |

每张反馈牌只保留四个信息位：

| 字段 | 说明 |
|------|------|
| `source_label` | 来源署名或机构类型，例如“皇后区读者”“科学会传真”“匿名广告主” |
| `quote` | 一句可读摘录，优先黑色幽默与现实错位 |
| `linked_article` | 对应本期版面、标题或公开解释 |
| `effect_tag` | 后果标签，例如“下周新线索”“势力关注”“地区异样”“声望上升” |

约束：

- 每期最多 3 张反馈牌。
- 每张最多承载 1 个明确效果。
- 至少 1 张可以引出下周钩子，但不要求每张都生成任务。
- 反馈牌写入 `settlement_result` 与 `next_week_hooks`，供下一周简报消费。
- 不做独立收件箱、逐封回信、读者好感度或长期读者关系模拟。

### 状态与切换

以下状态均发生在 `周循环与状态.summary` 内部。

| 状态 | 进入条件 | 退出条件 | 行为 |
|------|---------|---------|------|
| `issue_evaluate` | 发刊确认 | 传播值与结算项计算完毕 | 读取最终周刊并计算后果 |
| `audience_resolve` | 基础结算完成 | 受众与势力反馈写回 | 处理销量、订阅与反应 |
| `world_feedback` | 反馈写回完成 | 玩家确认下一周 | 展示宏观属性与现实偏移变化 |

### 与其他系统的交互

- **编辑排版与发行策略** 提供最终版面和刊登稿件。
- **四大势力任务、干预与声望** 提供任务奖励与惩罚模板。
- **宏观属性与现实偏移** 消费稿件属性和任务结果，更新世界状态。
- **同题公开取向历史** 在本系统中完成更新，供后续周次的排版预览与需求惩罚使用。
- **周循环与状态** 接收订阅、资源、`settlement_result` 和 `next_week_hooks`，并在玩家确认后进入下一周 `briefing`。

### 结算输入契约

结算阶段默认读取 `published_issue` 中冻结的下列关键字段：

| 字段 | 来源 | 用途 |
|------|------|------|
| `placed_articles[*].effective_base_value` | 排版系统 | 计算本期传播价值 |
| `placed_articles[*].intrinsic_base_value` | 排版系统 | 显示同题疲劳折损 |
| `placed_articles[*].public_stance` | 排版系统 | 计算取向冲突惩罚 |
| `placed_articles[*].primary_topic_key` | 排版系统 | 追踪同题历史 |
| `stats_preview` | 排版系统 | 与最终结算做对照展示 |

## 公式

### 发刊覆盖值

```
issue_reach =
  (base_reach + reputation_scale + issue_value + audience_modifier)
  * stance_clash_multiplier
```

| 变量 | 类型 | 范围 | 来源 | 说明 |
|------|------|------|------|------|
| `base_reach` | int | 0+ | 周刊状态 | 周刊的基础覆盖 |
| `reputation_scale` | int | 0+ | 声望属性 | 声望带来的天然扩散能力 |
| `issue_value` | int | 0+ | 排版系统 | 本期周刊综合传播价值 |
| `audience_modifier` | int | 负到正 | 受众计算 | 受众匹配带来的修正 |
| `stance_clash_multiplier` | float | 0.72-1.0 | 排版系统 | 同题公开取向冲突带来的需求折算 |

说明：

- `issue_value` 默认已经吃进同题疲劳折算后的有效基础值
- `stance_clash_multiplier` 只在本期存在同题公开取向冲突时小于 1

### 利润

```
profit = sales_income + sponsor_income - print_cost - operation_cost - penalty_cost
```

| 变量 | 类型 | 范围 | 来源 | 说明 |
|------|------|------|------|------|
| `sales_income` | float | 0+ | 销量结果 | 售出收入 |
| `sponsor_income` | float | 0+ | 财团/广告 | 额外商业收入 |
| `print_cost` | float | 0+ | 发刊配置 | 印刷成本 |
| `operation_cost` | float | 0+ | 周状态 | 编辑部运营成本 |
| `penalty_cost` | float | 0+ | 任务/违规 | 罚款、撤资等额外损失 |

## 边界情况

| 场景 | 预期行为 | 理由 |
|------|----------|------|
| 周刊勉强成刊但质量很差 | 允许发刊，并产生可学习的坏结果 | 支持失败叙事 |
| 同时命中多个势力任务 | 奖惩可叠加，但反馈必须分项展示 | 避免黑箱 |
| 零利润或亏损 | 继续进入下一周，但状态更差 | 保持长线张力 |
| 发刊造成重大现实偏移 | 必须在结算面板中显式展示 | 突出项目独特点 |
| 本期存在同题公开取向冲突 | 需求必须按冲突倍率折算，并在结果面板单独说明 | 让立场代价可读 |

## 依赖

| 系统 | 方向 | 依赖性质 |
|------|------|----------|
| 编辑排版与发行策略 | 本系统依赖它 | 提供本期周刊 |
| 四大势力任务、干预与声望 | 双向依赖 | 读取任务后果并写回关系变化 |
| 宏观属性与现实偏移 | 双向依赖 | 消费结算结果并输出世界变化 |
| 周循环与状态 | 双向依赖 | 写回下一周基础状态 |

## 调参旋钮

| 参数 | 当前值 | 安全范围 | 提高后的效果 | 降低后的效果 |
|------|--------|----------|--------------|--------------|
| `base_reach` | 中 | 低到中高 | 周刊更稳定 | 早期更脆弱 |
| 声望扩散倍率 | 中 | 低到高 | 声望更重要 | 稿件质量更重要 |
| 惩罚成本强度 | 中 | 低到高 | 违规更痛 | 玩家更敢冒险 |

## 验收标准

- [ ] 发刊后会一次性返回销量、利润、势力后果和宏观属性变化。
- [ ] 结算只消费本期周刊结构，不反向改写稿件。
- [ ] 结算能区分“同题疲劳导致的基础值折损”和“公开取向冲突导致的需求惩罚”。
- [ ] 至少有一种现实偏移结果会被清楚展示给玩家。
- [ ] 势力任务成功和失败的后果可被明确区分。

## 开放问题

| 问题 | 负责人 | 截止时间 | 结论 |
|------|--------|----------|------|
| 财团是否在 MVP 就提供独立的广告或赞助收益轨？ | 设计 | 后续 | 待定 |
| 受众层是否在 MVP 就拆成多群体，还是先以少量聚合指标呈现？ | 设计 | 后续 | 待定 |
