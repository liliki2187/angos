(function () {
  ...
  function randomWeekEvent() {
    const pool = [
      { title: "回合随机：线人爆料", body: "有人在暗网兜售「禁区坐标碎片」。" },
      { title: "回合随机：科学会通告", body: "科学会邀请你参加线上辟谣会。" },
      { title: "回合随机：印刷厂延误", body: "本周截稿压力上升。" },
      { title: "回合随机：读者来信潮", body: "都市传说类订阅上升。" },
    ];
  ...
  function renderGlobal() {
    ...
    grid.innerHTML = REGIONS.map((r) => {
      ...
      return `<div class="${cls}" data-region="${r.id}">
        <div class="name">${escapeHtml(r.name)}${r.unlocked && has ? " · 目标 " + visibleNodes.length : ""}</div>
        <div class="hint">${escapeHtml(r.hint)}</div></div>`;
    }).join("");
  ...
  function renderRegion() {
    ...
    const list = r.nodes
      ...
      .map((n) => {
        ...
        const deadline = n.kind === "temp" && n.deadlineDay != null ? `<span class="meta">截止：第 ${n.deadlineDay} 天前</span>` : "";
        const chain = n.chain === "locked" ? `<span class="meta">链式未解锁</span>` : "";
        const style = match ? "" : 'style="opacity:0.35"';
        const diff = n.difficulty || "normal";
        const diffZh = diff === "easy" ? "简单" : diff === "hard" ? "困难" : "普通";
        return `<div class="node ${hiddenCls}" data-node="${n.id}" ${style}>
          <div><div style="display:flex;gap:0.4rem;flex-wrap:wrap;align-items:center;">
            <strong>${escapeHtml(n.name)}</strong>
            <span class="badge ${n.kind === "permanent" ? "perm" : n.kind === "temp" ? "temp" : "hid"}">${n.kind === "permanent" ? "常驻" : n.kind === "temp" ? "临时" : "隐藏"}</span>
            <span class="badge" style="color:var(--muted)">${diffZh}</span></div>
            <div class="meta">耗时 ${n.days} 天 · 对手骰 ${n.enemyAttr | 0}</div>${deadline}${chain}</div>
          <button type="button" class="primary" ${!match || n.chain === "locked" ? "disabled" : ""}>配置队伍</button></div>`;
      })
      .join("");
    elR.innerHTML = `<h2>${escapeHtml(r.name)}</h2><div class="nodes">${list}</div><p><button type="button" id="backRegion">返回全球</button></p>`;
    elR.querySelectorAll(".node").forEach((nodeEl) => {
  ...
  function renderSetup() {
    ...
    pick.innerHTML = STAFF.map((p) => {
      ...
      return `<div class="staff-card ${sel ? "selected" : ""}" data-staff="${p.id}">
        ...
        <dl><dt>探索</dt><dd>${p.探索}</dd><dt>生存</dt><dd>${p.生存}</dd><dt>洞察</dt><dd>${p.洞察}</dd><dt>诡思</dt><dd>${p.诡思}</dd><dt>理性</dt><dd>${p.理性}</dd></dl></div>`;
    }).join("");
  ...
  function runMission() {
    ...
    const diceBlock = showDice
      ? `<div>${renderDiceRow(check.rollsA, new Set(), "调查池")}${renderDiceRow(check.rollsB, new Set(), "现场池")}
         <div class="phase-label" style="margin-top:0.5rem;">对手作废 ${check.enemyAttr} 颗</div>
         ${renderDiceRow(check.rollsA, check.voidA, "调查·作废后")}${renderDiceRow(check.rollsB, check.voidB, "现场·作废后")}</div>`
      : `<div class="prob-box">调查 ${check.hitsA}/${check.nAe}（≥${check.kA}）· 现场 ${check.hitsB}/${check.nBe}（≥${check.kB}）</div>`;
    elRes.innerHTML = `
      <h2>探索结算</h2>
      <div class="result-banner">${escapeHtml(tier)}</div>${diceBlock}
    ...
    document.getElementById("btnAfterResult").onclick = () => {
      ...
      state.selectedStaffIds = [];
      renderMacro();
      if (state.day <= 0) enterEditorialPhase();
      else {
        renderRegion();
        setView("region");
      }
  ...
  function buildEditorialPool() {
    const target = Math.max(Number(document.getElementById("storyCount").value), state.pendingClues.length);
    let cur = state.nextStoryId;
    const exploreStories = state.pendingClues.map((c) => clueToStory(c, cur++));
    const filler = [];
  ...
  function enterEditorialPhase() {
    ...
    document.getElementById("editorialSub").textContent = `第 ${state.week} 周 · 探索线索已转为报道条目（${state.pendingClues.length} 条），请组版后结算。`;
  ...
  function renderSlots() {
    ...
    for (const slot of slots) {
      ...
      div.className = "nm-slot";
      div.dataset.slotId = slot.id;
      const removeBtn = current ? `<button class="nm-sec" data-remove="${slot.id}">移除</button>` : "";
      div.innerHTML = `
        <div class="nm-slot-header"><span>${slot.name}</span><span>加成 ${(1 + 2 * slot.weight).toFixed(1)}x</span></div>
        <div style="font-size:12px;color:#94a3b8;">${slot.desc}</div>
        ${current ? `<div><strong>${escapeHtml(current.title)}</strong></div><div class="nm-chips"><span class="nm-chip">标签:${current.tags.map(toZhTag).join("/")}</span></div>` : `<div style="font-size:12px;color:#94a3b8;">暂无</div>`}
        <div class="nm-impact" data-impact="1">拖入报道可预览该版位影响</div>
        <div>${removeBtn}</div>`;
      div.addEventListener("dragover", (ev) => {
        ...
        const placedStories = slots
          .map((s) => (s.id === slot.id ? state.stories.find((x) => x.id === state.draggingStoryId) : state.placed[s.id]))
          .filter(Boolean);
        const effects = analyzeTagEffects(placedStories);
        const extra = [];
        if (effects.linked.length) extra.push(`连携奖励: ${effects.linked.map(toZhTag).join("、")}`);
        if (effects.dominantRatio > 0.7) extra.push(`偏科减益: ${effects.dominantType} ${Math.round(effects.dominantRatio * 100)}%`);
        ...
        impactEl.className = `nm-impact ${profitDelta > 0 ? "good" : profitDelta < 0 ? "bad" : "warn"}`;
  ...
  function settlePaper() {
    ...
    document.getElementById("summaryText").innerHTML = `
      ...
      本周探索转化稿件 <strong>${state.pendingClues.length}</strong> 条（已计入故事库）。`;
