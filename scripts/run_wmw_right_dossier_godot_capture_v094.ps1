...
$captureScript = 'res://tests/capture_world_map_wmw_right_dossier_runtime_v094.gd'
...
Write-Host 'Running WMW v0.9.4 right-dossier candidate A4 capture (windowed opengl3)...'
& $godot --path $projectPath --resolution 1920x1080 --windowed --audio-driver Dummy --rendering-driver opengl3 -s $captureScript
if ($LASTEXITCODE -ne 0) {
    throw 'WMW v0.9.4 right-dossier candidate A4 Godot capture failed.'
}

$expected = @(
    '615-world-map-wmw-v0-9-4-right-dossier-candidate-a4-godot-runtime.png',
    '616-world-map-wmw-v0-9-4-right-dossier-candidate-a4-godot-runtime-qa.png'
)
...
Write-Host 'WMW v0.9.4 right-dossier candidate A4 Godot capture passed.'
