...
$exportScript = 'res://tests/export_world_map_wmw_right_dossier_runtime_fixture_v093.gd'
$fixturePath = Join-Path $repoRoot 'gd_project\Assets\ui\angus_packaging\world_map\wmw_v093_right_dossier_candidate_a3\right_dossier_candidate_a3_runtime_fixture.json'
...
Write-Host 'Exporting WMW v0.9.3 right-dossier production runtime fixture (logic-only headless)...'
& $godot --headless --path $projectPath -s $exportScript
if ($LASTEXITCODE -ne 0) {
    throw 'WMW v0.9.3 right-dossier semantic fixture export failed.'
}
...
Write-Host 'WMW v0.9.3 right-dossier production runtime fixture passed.'
