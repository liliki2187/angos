...
$captureScript = 'res://tests/capture_world_map_wmw_right_dossier_runtime_v091.gd'
...
Write-Host 'Running WMW v0.9.1 right-dossier candidate A1 capture (windowed opengl3)...'
& $godot --path $projectPath --resolution 1920x1080 --windowed --audio-driver Dummy --rendering-driver opengl3 -s $captureScript
if ($LASTEXITCODE -ne 0) {
    throw 'WMW v0.9.1 right-dossier candidate A1 Godot capture failed.'
}

$expected = @(
    '594-world-map-wmw-v0-9-1-right-dossier-candidate-a1-godot-runtime.png',
    '595-world-map-wmw-v0-9-1-right-dossier-candidate-a1-godot-runtime-qa.png'
)
...
Write-Host 'WMW v0.9.1 right-dossier candidate A1 Godot capture passed.'
