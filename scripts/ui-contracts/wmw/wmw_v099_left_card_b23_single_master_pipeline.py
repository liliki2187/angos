...
VERSION = "v0.9.9"
ROUND_ID = "B2.3"
ASSET_ID = "world_map_wmw_left_region_card_b2_3_single_master_hollow_shell"

B_ATLAS = b22.B_ATLAS
B13_ATLAS = b22.B13_ATLAS

OUT_COMPARE = BASE / "459-world-map-wmw-v0-9-9-left-card-b2-3-single-master-derive-compare.png"
OUT_INGREDIENTS = BASE / "460-world-map-wmw-v0-9-9-left-card-b2-3-single-master-ingredients.png"
OUT_ATLAS = BASE / "461-world-map-wmw-v0-9-9-left-card-b2-3-atlas-2x.png"
OUT_GEOMETRY_QA = BASE / "462-world-map-wmw-v0-9-9-left-card-b2-3-geometry-qa.png"
OUT_RUNTIME = BASE / "463-world-map-wmw-v0-9-9-left-card-b2-3-runtime-fill.png"
OUT_RUNTIME_QA = BASE / "464-world-map-wmw-v0-9-9-left-card-b2-3-runtime-fill-qa.png"
OUT_FIT_QA = BASE / "465-world-map-wmw-v0-9-9-left-card-b2-3-300pct-fit-qa.png"
OUT_MANIFEST = BASE / "466-world-map-wmw-v0-9-9-left-card-b2-3-manifest.json"
OUT_GODOT = BASE / "467-world-map-wmw-v0-9-9-left-card-b2-3-godot-single-component.png"
OUT_GODOT_QA = BASE / "468-world-map-wmw-v0-9-9-left-card-b2-3-godot-single-component-qa.png"
...
GODOT_ATLAS = GODOT_ASSET_DIR / "left_region_card_b23_single_master_atlas_2x.png"
GODOT_MANIFEST = GODOT_ASSET_DIR / "left_region_card_b23_single_master_manifest.json"
...
def _frame_family_mask_pixel(c: tuple[int, int, int, int], x: int, y: int) -> bool:
    ...
    if _is_globe_area(x, y) and (r > 110 and g > 95 and b > 70):
        return False
    # The available master frame family is teal/cyan or dark teal. This mask
    # deliberately excludes cream paper, globe ink, badge base, and cutout.
...
def paste_badge_slot(shell: Image.Image, source_state_frame: Image.Image, state: str) -> Image.Image:
    if state == MASTER_STATE:
        return shell
    x1, y1, x2, y2 = ACTION_RECT_2X
    out = shell.copy().convert("RGBA")
    slot = source_state_frame.convert("RGBA").crop(ACTION_RECT_2X)
    out.alpha_composite(slot, (x1, y1))
    return out
...
def make_master_hollow_shell(master_frame: Image.Image, window: tuple[int, int, int, int], globe: dict) -> tuple[Image.Image, Image.Image, dict]:
    shell = master_frame.copy().convert("RGBA")
    mask = b22.cutout_mask_for(window, globe)
    px = shell.load()
    mp = mask.load()
    cut_pixels = 0
    for y in range(FRAME_SIZE[1]):
    ...
    return shell, mask, {
        ...
        "method": "single available-state master; pure geometric cutout = measured window rectangle minus available globe disc",
    }
...
def compose_single_master_frames() -> tuple[list[Image.Image], list[Image.Image], Image.Image, Image.Image, dict, list[dict], list[dict]]:
    ...
    window, measurement_meta = b22.measure_window(master_frame)
    globes = b22.measure_globes(source_frames)
    globe = globes[MASTER_INDEX]
    master_shell, cutout_mask, master_meta = make_master_hollow_shell(master_frame, window, globe)
    ...
    for idx, state in enumerate(STATE_ORDER):
        if state == MASTER_STATE:
            ...
            shell = paste_badge_slot(shell, source_frames[idx], state)
        ...
        per_state.append(
            {
                ...
                "badge_icon_source": "available master" if state == MASTER_STATE else f"candidate B original {state} action_badge slot crop",
                "selected_glow_layer": "selected outer green glow extracted from original selected frame" if state == "selected" else "not_applied",
                "z_order": "transparent base -> rectangular B1.3 photo underlay -> single-master derived hollow shell -> runtime text",
                "gates": gates,
            }
        )
    ...
    measurement = {
        "master_state": MASTER_STATE,
        "measurement_rule_version": "wmw_hollow_shell_photo_pipeline_v1_1_single_master",
        "master_window_rect_2x": list(window),
    ...
    return frames, shells, master_shell, cutout_mask, measurement, per_state, [
        {
            "state": state,
...
def make_compare_board(source_frames: list[Image.Image], derived_frames: list[Image.Image], measurement: dict) -> Image.Image:
    ...
    draw.text((42, 24), "B2.3 v0.9.9 single-master derivation QA", fill=(242, 238, 210), font=b11.F_HEAD)
    draw.text((42, 64), f"master={MASTER_STATE} window={measurement['master_window_rect_2x']}  GateE=diff 0 by derivation", fill=(205, 224, 198), font=b11.F_NOTE)
    for idx, state in enumerate(STATE_ORDER):
        x = 42 + idx * 420
        y = 112
    ...
    draw.text((42, 760), "Manual art check: derived color/material should not be obviously worse than original B state frames. If it is worse, stop before production freeze.", fill=(235, 206, 158), font=b11.F_NOTE)
    return canvas


def make_ingredient_board(shells: list[Image.Image], master_shell: Image.Image, mask: Image.Image, measurement: dict) -> Image.Image:
    canvas = Image.new("RGBA", (1680, 1060), (9, 22, 24, 255))
    draw = ImageDraw.Draw(canvas)
    draw.text((42, 24), "B2.3 single-master ingredients", fill=(242, 238, 210), font=b11.F_HEAD)
    draw.text((42, 64), "available hollow shell is the only geometry source; other states are hue/icon/glow derivations.", fill=(205, 224, 198), font=b11.F_NOTE)
    headers = ["master hollow shell", "cutout mask", "selected", "available", "warning", "locked"]
    for i, header in enumerate(headers):
        draw.text((42 + i * 270, 104), header, fill=(188, 216, 190), font=b11.F_SMALL)
    previews = [
        master_shell,
...
def make_fit_qa(frames: list[Image.Image], measurement: dict) -> Image.Image:
    ...
    draw = ImageDraw.Draw(canvas)
    draw.text((42, 24), "B2.3 v0.9.9 300% fit QA: single master window for all states", fill=(242, 238, 210), font=b11.F_HEAD)
    draw.text((42, 64), "Required: globe arc / right edge / bottom edge show no seam, old band, ghost, or non-selected green residue.", fill=(205, 224, 198), font=b11.F_NOTE)
    for idx, state in enumerate(STATE_ORDER):
        row_y = 122 + idx * 318
        bg = Image.new("RGBA", FRAME_SIZE, (7, 19, 21, 255))
        ...
        for panel_idx, (title, crop) in enumerate(
            ...
            x = [42, 505, 800][panel_idx]
            panel = bg.crop(crop).resize(((crop[2] - crop[0]) * 3, (crop[3] - crop[1]) * 3), Image.Resampling.NEAREST)
...
def make_runtime_preview(frames: list[Image.Image], show_qa: bool) -> Image.Image:
    ...
    draw.text((450, 34), "Python v0.9.9 B2.3 left_region_card runtime fill", fill=(243, 239, 214), font=b11.F_HEAD)
    draw.text((450, 76), "Single available master hollow shell; rectangular photos below; title/meta are runtime text tokens.", fill=(217, 222, 199), font=b11.F_NOTE)
    return canvas
...
def aggregate_gates(per_state: list[dict], measurement: dict, manual_pass: bool, godot_pass: bool) -> dict:
    ...
    return {
        ...
        "manual_300pct_visual_check": {
            ...
            "basis": "inspect 465: every state globe arc/right edge/bottom edge must be clean",
            "evidence": str(OUT_FIT_QA),
        },
        "derived_art_quality_check": {
            "status": "pass" if manual_pass else "pending",
            "basis": "inspect 459: derived status colors/material should not be obviously worse than source B frames",
...
def make_manifest(measurement: dict, per_state: list[dict], geometry_metrics: list[dict], checks: dict, manual_pass: bool, godot_pass: bool) -> dict:
    ...
    full_pass = all(gate["status"] == "pass" for gate in gates.values())
    return {
        "schema_version": 1,
        "asset_id": ASSET_ID,
        "version": VERSION,
        "round": ROUND_ID,
        "date": "2026-07-09",
        "status": "b2_3_single_master_pass_pending_user_review" if full_pass else "b2_3_single_master_pending_full_chain",
        "contract": str(b11.CONTRACT_PATH),
        "contract_frozen_fields_changed": False,
        "pipeline": "available single master -> pure geometric hollow shell -> derived state skins -> rectangular photo underlay -> runtime text",
        "inputs": {
...
def write_outputs(manual_pass: bool = False) -> dict:
    source_frames = b22.load_atlas_frames(B_ATLAS)
    frames, shells, master_shell, mask, measurement, per_state, geometry_metrics = compose_single_master_frames()
    OUT_COMPARE.parent.mkdir(parents=True, exist_ok=True)
    make_compare_board(source_frames, frames, measurement).save(OUT_COMPARE)
    make_ingredient_board(shells, master_shell, mask, measurement).save(OUT_INGREDIENTS)
    ...
    for state, shell in zip(STATE_ORDER, shells):
        shell.save(GODOT_INGREDIENT_DIR / f"left_region_card_b23_single_master_hollow_shell_{state}.png")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build WMW left card B2.3 single-master hollow-shell atlas.")
    parser.add_argument(
        "--manual-ok",
        action="store_true",
        help="Mark 459/465 manual visual checks as pass after human inspection.",
    )
