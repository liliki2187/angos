...
VERSION = "v0.9.15"
ROUND_ID = "B2.9"
ASSET_ID = "world_map_wmw_left_region_card_b2_9_no_inpaint_badge"

OUT_COMPARE = BASE / "519-world-map-wmw-v0-9-15-left-card-b2-9-no-inpaint-compare.png"
OUT_INGREDIENTS = BASE / "520-world-map-wmw-v0-9-15-left-card-b2-9-ingredients-qa.png"
OUT_ATLAS = BASE / "521-world-map-wmw-v0-9-15-left-card-b2-9-atlas-2x.png"
OUT_GEOMETRY = BASE / "522-world-map-wmw-v0-9-15-left-card-b2-9-geometry-qa.png"
OUT_RUNTIME = BASE / "523-world-map-wmw-v0-9-15-left-card-b2-9-runtime-fill.png"
OUT_RUNTIME_QA = BASE / "524-world-map-wmw-v0-9-15-left-card-b2-9-runtime-fill-qa.png"
OUT_VISUAL_QA = BASE / "525-world-map-wmw-v0-9-15-left-card-b2-9-16point-visual-qa.png"
OUT_MANIFEST = BASE / "526-world-map-wmw-v0-9-15-left-card-b2-9-manifest.json"
OUT_GODOT = BASE / "527-world-map-wmw-v0-9-15-left-card-b2-9-godot-single-component.png"
OUT_GODOT_QA = BASE / "528-world-map-wmw-v0-9-15-left-card-b2-9-godot-single-component-qa.png"
...
GODOT_ATLAS = GODOT_ASSET_DIR / "left_region_card_b29_no_inpaint_badge_atlas_2x.png"
GODOT_MANIFEST = GODOT_ASSET_DIR / "left_region_card_b29_no_inpaint_badge_manifest.json"
...
# Candidate B's exterior ring is already the approved geometry. B2.9 never
...
def neutral_core_texture(shell: Image.Image) -> Image.Image:
    ...
    draw.polygon([(310, 216), (354, 216), (332, 246)], fill=tone(6))
    draw.polygon([(354, 216), (361, 269), (332, 246)], fill=tone(1))
    draw.polygon([(361, 269), (310, 276), (332, 246)], fill=tone(-6))
    draw.polygon([(310, 276), (302, 224), (332, 246)], fill=tone(-2))
    px = out.load()
...
def complete_runtime_icon(source: Image.Image, state: str) -> tuple[Image.Image, dict]:
    ...
    purity = b26.ingredient_purity_gate(canvas, "cream_icon", f"b29_runtime_icon_{state}")
...
def make_compare_board(data: dict) -> Image.Image:
    ...
    draw.text((42, 24), "B2.9 badge surface: candidate B -> failed B2.8 inpaint -> preserved exterior", fill=(243, 239, 214), font=b11.F_HEAD)
    draw.text((42, 66), "Candidate B exterior ring and right frame remain pixel-identical; only the enclosed semantic core is rebuilt.", fill=(207, 224, 199), font=b11.F_NOTE)
    b28_frames = b22.load_atlas_frames(b28.OUT_ATLAS)
    ...
    for idx, state in enumerate(STATE_ORDER):
        ...
        rows = [
            ...
            ("B2.8 exposed harmonic inpaint", b28_frames[idx], (255, 120, 120)),
            ("B2.9 source exterior + clean core", data["frames"][idx], (120, 235, 170)),
        ]
    ...
    draw.text((42, 1280), f"B2.9 outer bbox={gate['outer_ring']['bbox']} bbox center={gate['outer_ring_bbox_center']} right gap={gate['right_visual_gap_1x']}px | outside-core changed={continuity['changed_pixels_outside_inner_opening']} | inpaint={continuity['harmonic_inpaint_called']}", fill=(235, 206, 158), font=b11.F_NOTE)
    return canvas
...
def make_ingredients_board(data: dict) -> Image.Image:
    ...
    draw = ImageDraw.Draw(canvas)
    draw.text((42, 24), "B2.9 no-inpaint ingredients dependency board", fill=(243, 239, 214), font=b11.F_HEAD)
    draw.text((42, 66), "Photos and globe stay pure; candidate B badge exterior is preserved; only the geometric inner opening changes.", fill=(207, 224, 199), font=b11.F_NOTE)
    ...
    draw.text((42, 1270), f"No-smear gate: harmonic_inpaint={continuity['harmonic_inpaint_called']} | changed outside inner opening={continuity['changed_pixels_outside_inner_opening']} | right corridor changed={continuity['changed_pixels_in_right_corridor_outside_inner_opening']} | exposed inpaint={continuity['exposed_inpaint_pixels']}", fill=(120, 235, 170), font=b11.F_NOTE)
    return canvas
...
def make_geometry_board(data: dict) -> Image.Image:
    ...
    draw = ImageDraw.Draw(canvas)
    draw.text((42, 24), "B2.9 geometry QA: preserved candidate-B exterior + contract v0.8.3", fill=(243, 239, 214), font=b11.F_HEAD)
...
def make_runtime_preview(data: dict, qa: bool) -> Image.Image:
    ...
    draw.text((450, 34), "Python v0.9.15 B2.9 no-inpaint state badge", fill=(243, 239, 214), font=b11.F_HEAD)
    draw.text((450, 76), "Candidate-B exterior is preserved pixel-for-pixel; only the semantic core and runtime icon are separate layers.", fill=(217, 222, 199), font=b11.F_NOTE)
...
def make_visual_board(data: dict) -> Image.Image:
    ...
    draw = ImageDraw.Draw(canvas)
    draw.text((42, 24), "B2.9 16-point no-smear evidence board", fill=(243, 239, 214), font=b11.F_HEAD)
...
def make_manifest(data: dict, checks: dict, godot_pass: bool, partial_frame_probe: dict) -> dict:
    ...
    return {
        ...
        "round": ROUND_ID,
        "date": "2026-07-10",
        "status": "b2_9_evidence_ready_pending_user_visual_review" if passed and godot_pass else "b2_9_pending_full_chain",
        ...
        "pipeline": "396 clean photo -> hollow shell retaining approved candidate-B badge exterior -> pure geometric inner-opening replacement -> selected glow -> independent globe linework -> runtime text + complete padded state glyph",
        "inputs": {
        ...
        "outputs": {
            ...
            "runtime_icons": {state: str(GODOT_INGREDIENT_DIR / f"left_region_card_b29_runtime_icon_{state}.png") for state in STATE_ORDER},
            "clean_photos": {state: str(GODOT_INGREDIENT_DIR / f"left_region_card_b29_clean_photo_{state}.png") for state in STATE_ORDER},
            "globe_linework": str(GODOT_INGREDIENT_DIR / "left_region_card_b29_globe_linework.png"),
        },
        "per_state": data["per_state"],
        "gates": gates,
...
def write_outputs(approve_ingredients: bool) -> dict:
    ...
    data["master_shell"].save(GODOT_INGREDIENT_DIR / "left_region_card_b29_clean_master.png")
    data["globe_patch"].save(GODOT_INGREDIENT_DIR / "left_region_card_b29_globe_linework.png")
    for state, shell in zip(STATE_ORDER, data["shells"]):
        shell.save(GODOT_INGREDIENT_DIR / f"left_region_card_b29_hollow_shell_{state}.png")
    for state in STATE_ORDER:
        data["icons"][state].save(GODOT_INGREDIENT_DIR / f"left_region_card_b29_runtime_icon_{state}.png")
        data["photos"][state].save(GODOT_INGREDIENT_DIR / f"left_region_card_b29_clean_photo_{state}.png")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build WMW left card B2.9 no-inpaint badge pipeline.")
    parser.add_argument("--approve-ingredients", action="store_true", help="Continue only after 519/520 no-smear dependency evidence has been inspected.")
