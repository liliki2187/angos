...
VERSION = "v0.9.10"
ROUND_ID = "B2.4"
ASSET_ID = "world_map_wmw_left_region_card_b2_4_single_master_overlay_fix"

B_ATLAS = b22.B_ATLAS
B13_ATLAS = b22.B13_ATLAS

OUT_COMPARE = BASE / "469-world-map-wmw-v0-9-10-left-card-b2-4-layer-fix-compare.png"
OUT_INGREDIENTS = BASE / "470-world-map-wmw-v0-9-10-left-card-b2-4-ingredients.png"
OUT_ATLAS = BASE / "471-world-map-wmw-v0-9-10-left-card-b2-4-atlas-2x.png"
OUT_GEOMETRY_QA = BASE / "472-world-map-wmw-v0-9-10-left-card-b2-4-geometry-qa.png"
OUT_RUNTIME = BASE / "473-world-map-wmw-v0-9-10-left-card-b2-4-runtime-fill.png"
OUT_RUNTIME_QA = BASE / "474-world-map-wmw-v0-9-10-left-card-b2-4-runtime-fill-qa.png"
OUT_FIT_QA = BASE / "475-world-map-wmw-v0-9-10-left-card-b2-4-16point-visual-qa.png"
OUT_MANIFEST = BASE / "476-world-map-wmw-v0-9-10-left-card-b2-4-manifest.json"
OUT_GODOT = BASE / "477-world-map-wmw-v0-9-10-left-card-b2-4-godot-single-component.png"
OUT_GODOT_QA = BASE / "478-world-map-wmw-v0-9-10-left-card-b2-4-godot-single-component-qa.png"
...
GODOT_ATLAS = GODOT_ASSET_DIR / "left_region_card_b24_overlay_fix_atlas_2x.png"
GODOT_MANIFEST = GODOT_ASSET_DIR / "left_region_card_b24_overlay_fix_manifest.json"
...
def extract_badge_glyph(source_state_frame: Image.Image) -> Image.Image:
    src = source_state_frame.convert("RGBA")
    sp = src.load()
    coords = sorted(badge_glyph_pixels(src))
    if not coords:
...
def paste_badge_glyph(shell: Image.Image, source_state_frame: Image.Image, state: str) -> Image.Image:
    if state == MASTER_STATE:
        return shell
    out = clear_badge_glyph_area(shell)
    glyph = extract_badge_glyph(source_state_frame)
    cx, cy = _badge_center()
...
def compose_single_master_frames() -> tuple[list[Image.Image], list[Image.Image], Image.Image, Image.Image, Image.Image, dict, list[dict], list[dict]]:
    ...
    for idx, state in enumerate(STATE_ORDER):
        if state == MASTER_STATE:
            ...
            shell = paste_badge_glyph(shell, source_frames[idx], state)
        ...
        per_state.append(
            {
                ...
                "badge_icon_source": "available master" if state == MASTER_STATE else f"candidate B original {state} cream glyph only, centered inside master badge ring",
    ...
    measurement = {
        "master_state": MASTER_STATE,
        "measurement_rule_version": "wmw_hollow_shell_photo_pipeline_v1_1_single_master_b2_4_edge_residue_fix",
        "base_measured_window_rect_2x": list(base_window),
        "master_window_rect_2x": list(window),
...
def make_compare_board(source_frames: list[Image.Image], derived_frames: list[Image.Image], measurement: dict) -> Image.Image:
    ...
    draw.text((42, 24), "B2.4 v0.9.10 layer-fix derivation QA", fill=(242, 238, 210), font=b11.F_HEAD)
    ...
    draw.text((42, 760), "B2.4 fixes: full window cutout, edge residue expansion, independent globe patch, glyph-only badge swap.", fill=(235, 206, 158), font=b11.F_NOTE)
    return canvas
...
def make_ingredient_board(shells: list[Image.Image], master_shell: Image.Image, mask: Image.Image, globe_patch: Image.Image, measurement: dict) -> Image.Image:
    ...
    draw = ImageDraw.Draw(canvas)
    draw.text((42, 24), "B2.4 single-master ingredients", fill=(242, 238, 210), font=b11.F_HEAD)
...
def make_fit_qa(frames: list[Image.Image], measurement: dict) -> Image.Image:
    ...
    draw = ImageDraw.Draw(canvas)
    draw.text((42, 24), "B2.4 v0.9.10 16-point visual QA: single master + overlay fixes", fill=(242, 238, 210), font=b11.F_HEAD)
...
def make_runtime_preview(frames: list[Image.Image], show_qa: bool) -> Image.Image:
    ...
    draw.text((450, 34), "Python v0.9.10 B2.4 left_region_card runtime fill", fill=(243, 239, 214), font=b11.F_HEAD)
    draw.text((450, 76), "Full window cutout, edge residue expansion, independent globe patch, glyph-only badge swap.", fill=(217, 222, 199), font=b11.F_NOTE)
    return canvas
...
def aggregate_gates(per_state: list[dict], measurement: dict, manual_pass: bool, godot_pass: bool) -> dict:
    ...
    return {
        ...
        "manual_16point_visual_check": {
            ...
            "basis": "inspect 475: four states x four checkpoints (globe arc/right edge/bottom edge/badge) must each pass; no batch pass wording",
            "evidence": str(OUT_FIT_QA),
            "checks": manual_checks,
        },
        "derived_art_quality_check": {
            "status": "pass" if manual_pass else "pending",
            "basis": "inspect 469: derived status colors/material should not be obviously worse than source B frames",
...
def make_manifest(measurement: dict, per_state: list[dict], geometry_metrics: list[dict], checks: dict, manual_pass: bool, godot_pass: bool) -> dict:
    ...
    return {
        ...
        "round": ROUND_ID,
        "date": "2026-07-09",
        "status": "b2_4_overlay_fix_pass_pending_user_review" if full_pass else "b2_4_overlay_fix_pending_full_chain",
        "contract": str(b11.CONTRACT_PATH),
        "contract_frozen_fields_changed": False,
        "pipeline": "available single master -> edge-expanded full-window cutout -> derived state skins -> rectangular photo underlay -> independent globe patch -> runtime text",
        "inputs": {
...
def write_outputs(manual_pass: bool = False) -> dict:
    ...
    globe_patch.save(GODOT_INGREDIENT_DIR / "left_region_card_b24_independent_globe_patch.png")
    for state, shell in zip(STATE_ORDER, shells):
        shell.save(GODOT_INGREDIENT_DIR / f"left_region_card_b24_overlay_fix_hollow_shell_{state}.png")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build WMW left card B2.4 overlay-fix single-master atlas.")
    parser.add_argument(
        "--manual-ok",
        action="store_true",
        help="Mark 475 16-point manual visual checks as pass after human inspection.",
    )
