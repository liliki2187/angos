...
VERSION = "v0.9.12"
ROUND_ID = "B2.6"
ASSET_ID = "world_map_wmw_left_region_card_b2_6_runtime_badge"

B_ATLAS = b22.B_ATLAS
B13_ATLAS = b22.B13_ATLAS

OUT_COMPARE = BASE / "489-world-map-wmw-v0-9-12-left-card-b2-6-runtime-badge-compare.png"
OUT_INGREDIENTS = BASE / "490-world-map-wmw-v0-9-12-left-card-b2-6-ingredients-purity-qa.png"
OUT_ATLAS = BASE / "491-world-map-wmw-v0-9-12-left-card-b2-6-atlas-2x.png"
OUT_GEOMETRY_QA = BASE / "492-world-map-wmw-v0-9-12-left-card-b2-6-geometry-qa.png"
OUT_RUNTIME = BASE / "493-world-map-wmw-v0-9-12-left-card-b2-6-runtime-fill.png"
OUT_RUNTIME_QA = BASE / "494-world-map-wmw-v0-9-12-left-card-b2-6-runtime-fill-qa.png"
OUT_FIT_QA = BASE / "495-world-map-wmw-v0-9-12-left-card-b2-6-16point-visual-qa.png"
OUT_MANIFEST = BASE / "496-world-map-wmw-v0-9-12-left-card-b2-6-manifest.json"
OUT_GODOT = BASE / "497-world-map-wmw-v0-9-12-left-card-b2-6-godot-single-component.png"
OUT_GODOT_QA = BASE / "498-world-map-wmw-v0-9-12-left-card-b2-6-godot-single-component-qa.png"
...
GODOT_ATLAS = GODOT_ASSET_DIR / "left_region_card_b26_runtime_badge_atlas_2x.png"
GODOT_MANIFEST = GODOT_ASSET_DIR / "left_region_card_b26_runtime_badge_manifest.json"
...
def _inside_badge_clear_zone(x: int, y: int) -> bool:
    cx, cy = _badge_center()
    return (x - cx) * (x - cx) + (y - cy) * (y - cy) <= BADGE_GLYPH_CLEAR_RADIUS * BADGE_GLYPH_CLEAR_RADIUS
...
def atlas_baked_state_glyph_pixels(img: Image.Image) -> set[tuple[int, int]]:
    return {
        (x, y)
        for x, y in badge_glyph_pixels(img)
        if _inside_badge_glyph_zone(x, y)
    }
...
def badge_glyph_clear_pixels(img: Image.Image) -> set[tuple[int, int]]:
    ...
    for y in range(y1, y2):
        for x in range(x1, x2):
            if _inside_badge_clear_zone(x, y) and (x, y) not in outer:
                clear.add((x, y))
    for gx, gy in glyph:
...
def _frame_family_mask_pixel(c: tuple[int, int, int, int], x: int, y: int) -> bool:
    ...
    if _is_globe_area(x, y) and (r > 110 and g > 95 and b > 70):
        return False
    # The available master frame family is teal/cyan or dark teal. B2.6
    # deliberately includes the clean badge core so state hue mapping owns it.
...
def gate_badge_state_color_consistency(frame: Image.Image, family_mask: Image.Image, state: str) -> dict:
    ...
    if frame_summary["hue"] is None or badge_summary["hue"] is None:
        ...
        if low_sat_mode:
            ...
            mode = "hue_compare"
    residual_glyph_pixels = len(atlas_baked_state_glyph_pixels(frame))
    return {
        "status": "pass" if enough_samples and color_consistent and residual_glyph_pixels == 0 else "fail",
        "state": state,
        "mode": mode,
        ...
        "atlas_baked_state_glyph_pixels": residual_glyph_pixels,
        "runtime_icon_policy": "badge state glyphs are forbidden in atlas; icons are drawn by Python/Godot runtime layer",
    }
...
def _is_globe_target_family(c: tuple[int, int, int, int]) -> bool:
    r, g, b, a = c
    return a > 0 and r > 110 and g > 90 and b > 60 and r > b + 14


def _is_icon_target_family(c: tuple[int, int, int, int]) -> bool:
    return c[3] > 0 and _is_creamish(c)


def ingredient_purity_gate(img: Image.Image, target: str, name: str) -> dict:
    px = img.convert("RGBA").load()
    opaque = 0
    non_target = 0
    ...
    for y in range(img.height):
        for x in range(img.width):
            ...
            if c[3] == 0:
                continue
            opaque += 1
            if target == "cream_icon":
                ok = _is_icon_target_family(c)
            elif target == "warm_globe":
                ok = _is_globe_target_family(c) and not _is_old_photo_signature(c)
            else:
    ...
    ratio = non_target / opaque if opaque else 1.0
    return {
        "status": "pass" if opaque > 0 and ratio <= INGREDIENT_NON_TARGET_RATIO_TOLERANCE else "fail",
        "ingredient": name,
        "target_family": target,
        "opaque_pixels": opaque,
        "non_target_pixels": non_target,
        "non_target_ratio": round(ratio, 6),
        "ratio_tolerance": INGREDIENT_NON_TARGET_RATIO_TOLERANCE,
        "sample_non_target_pixels": samples,
...
def extract_globe_patch(master_frame: Image.Image) -> Image.Image:
    ...
    for y in range(0, min(112, FRAME_SIZE[1])):
        for x in range(0, min(112, FRAME_SIZE[0])):
            if b22.is_cream(sp[x, y]):
                pp[x, y] = 255
    expanded = primary.filter(ImageFilter.MaxFilter(3))
    ep = expanded.load()
    ...
    for y in range(0, min(112, FRAME_SIZE[1])):
        for x in range(0, min(112, FRAME_SIZE[0])):
            c = sp[x, y]
            if pp[x, y] or (ep[x, y] and _is_globe_antialias(c) and _is_globe_target_family(c)):
                qp[x, y] = c
    return patch


def extract_runtime_badge_icon(source_state_frame: Image.Image, state: str) -> tuple[Image.Image, dict]:
    src = source_state_frame.convert("RGBA")
    sp = src.load()
    raw_coords = sorted(badge_glyph_pixels(src))
    coords = [coord for coord in raw_coords if _is_icon_target_family(sp[coord[0], coord[1]])]
    if not coords:
        return Image.new("RGBA", (1, 1), (0, 0, 0, 0)), {
            "status": "fail",
            "state": state,
            "raw_cream_glyph_pixels": len(raw_coords),
            "runtime_icon_pixels": 0,
            "non_cream_opaque_pixels": 0,
            "transparent_background_pixels": 1,
            "method": "cream glyph components excluding master outer badge ring; transparent runtime icon ingredient",
        }
    bx1 = min(x for x, _y in coords)
    by1 = min(y for _x, y in coords)
    bx2 = max(x for x, _y in coords) + 1
    by2 = max(y for _x, y in coords) + 1
    icon = Image.new("RGBA", (bx2 - bx1, by2 - by1), (0, 0, 0, 0))
    gp = icon.load()
    for x, y in coords:
    ...
    return icon, {
        ...
        "state": state,
        "raw_cream_glyph_pixels": len(raw_coords),
        "runtime_icon_pixels": len(coords),
        "non_cream_opaque_pixels": non_cream,
        "transparent_background_pixels": icon.width * icon.height - opaque,
        "crop_bbox_2x": [bx1, by1, bx2, by2],
        "method": "cream glyph components excluding master outer badge ring; transparent runtime icon ingredient; no atlas baking and no rectangular background retained",
...
def compose_single_master_frames() -> tuple[list[Image.Image], list[Image.Image], Image.Image, Image.Image, Image.Image, dict[str, Image.Image], dict, dict, list[dict], list[dict]]:
    ...
    master_shell = clear_badge_glyph_area(master_shell)
    pre_recolor_old_content_gate = gate_old_content_leftover(master_shell, globe_patch, window)
    ...
    runtime_icon_metrics: dict[str, dict] = {}
    for idx, state in enumerate(STATE_ORDER):
        icon, metrics = extract_runtime_badge_icon(source_frames[idx], state)
        runtime_icons[state] = icon
        purity = ingredient_purity_gate(icon, "cream_icon", f"runtime_badge_icon_{state}")
        metrics["ingredient_purity"] = purity
        runtime_icon_metrics[state] = metrics
    globe_purity = ingredient_purity_gate(globe_patch, "warm_globe", "independent_globe_patch")
    ingredient_purity = {
        "status": "pass" if globe_purity["status"] == "pass" and all(item["ingredient_purity"]["status"] == "pass" for item in runtime_icon_metrics.values()) else "fail",
        "globe_patch": globe_purity,
        "runtime_badge_icons": runtime_icon_metrics,
        "manual_400pct_review": {
    ...
    for idx, state in enumerate(STATE_ORDER):
        ...
        gates = {
            ...
            "old_content_leftover_scan": pre_recolor_old_content_gate,
            ...
            "badge_state_color_consistency": gate_badge_state_color_consistency(frame, family_mask, state),
        ...
        per_state.append(
            {
                ...
                "badge_icon_source": f"runtime icon ingredient extracted from candidate B original {state}; not baked into atlas",
                "runtime_badge_icon": runtime_icon_metrics[state],
                "selected_glow_layer": "selected outer green glow extracted from original selected frame" if state == "selected" else "not_applied",
                "z_order": "transparent base -> rectangular B1.3 photo underlay -> clean single-master derived hollow shell -> selected glow if selected -> independent globe patch -> runtime text + runtime badge icon",
                "gates": gates,
            }
        )
    ...
    measurement = {
        "master_state": MASTER_STATE,
        "measurement_rule_version": "wmw_hollow_shell_photo_pipeline_v1_1_single_master_b2_6_runtime_badge",
        "base_measured_window_rect_2x": list(base_window),
        "master_window_rect_2x": list(window),
        "master_adaptive_classifier_samples": measurement_meta,
        "master_globe_disc": globe,
        "globe_layering": "independent warm-cream globe patch; window cutout has no globe protection zone; ingredient_purity checked before composition",
        "badge_layering": "clean badge base in atlas; state glyphs are runtime icon ingredients and are not baked",
        "old_content_leftover_scan_timing": "pre_state_recolor_only",
        "ingredient_purity": ingredient_purity,
        "window_edge_residue_scan": edge_residue_scan,
    ...
    return frames, shells, master_shell, cutout_mask, globe_patch, runtime_icons, measurement, per_state, [
        {
            "state": state,
...
def make_compare_board(source_frames: list[Image.Image], derived_frames: list[Image.Image], measurement: dict) -> Image.Image:
    ...
    draw.text((42, 24), "B2.6 v0.9.12 runtime-badge derivation QA", fill=(242, 238, 210), font=b11.F_HEAD)
    ...
    draw.text((42, 760), "B2.6 fixes: atlas badge base is clean; state glyphs move to runtime icon layer.", fill=(235, 206, 158), font=b11.F_NOTE)
    return canvas


def make_ingredient_board(shells: list[Image.Image], master_shell: Image.Image, mask: Image.Image, globe_patch: Image.Image, runtime_icons: dict[str, Image.Image], measurement: dict) -> Image.Image:
    canvas = Image.new("RGBA", (1920, 1260), (9, 22, 24, 255))
    draw = ImageDraw.Draw(canvas)
    draw.text((42, 24), "B2.6 single-master ingredients + purity QA", fill=(242, 238, 210), font=b11.F_HEAD)
    draw.text((42, 64), "Atlas uses clean badge bases; globe is a frozen static patch; badge state icons are runtime PNG ingredients.", fill=(205, 224, 198), font=b11.F_NOTE)
    ...
    draw.text((42, 390), "400% runtime badge icon ingredients", fill=(242, 238, 210), font=b11.F_NOTE)
    for i, state in enumerate(STATE_ORDER):
        icon = runtime_icons[state]
        x = 42 + i * 280
        y = 430
        preview_box = 230
        bg = Image.new("RGBA", (preview_box, preview_box), (7, 19, 21, 255))
        scale = 4
        preview = icon.resize((max(1, icon.width * scale), max(1, icon.height * scale)), Image.Resampling.NEAREST)
        bg.alpha_composite(preview, ((preview_box - preview.width) // 2, (preview_box - preview.height) // 2))
        canvas.alpha_composite(bg, (x, y))
        draw.rectangle((x, y, x + preview_box, y + preview_box), outline=(94, 164, 142), width=2)
        purity = measurement["ingredient_purity"]["runtime_badge_icons"][state]["ingredient_purity"]
        draw.text((x, y - 24), state, fill=(255, 229, 93), font=b11.F_SMALL)
        draw.text((x, y + preview_box + 8), f"opaque={purity['opaque_pixels']} non-target={purity['non_target_pixels']}", fill=(205, 224, 198), font=b11.F_SMALL)
    draw.text((42, 720), "400% globe patch ingredient", fill=(242, 238, 210), font=b11.F_NOTE)
    globe_box = Image.new("RGBA", (360, 240), (7, 19, 21, 255))
    globe_crop = globe_patch.crop((0, 0, 112, 112)).resize((448, 448), Image.Resampling.NEAREST)
    globe_box.alpha_composite(globe_crop, (-44, -92))
    canvas.alpha_composite(globe_box, (42, 760))
    gp = measurement["ingredient_purity"]["globe_patch"]
    draw.rectangle((42, 760, 402, 1000), outline=(94, 164, 142), width=2)
    draw.text((430, 760), f"globe opaque={gp['opaque_pixels']} non-target={gp['non_target_pixels']}", fill=(205, 224, 198), font=b11.F_SMALL)
    draw.text((42, 1040), json.dumps(measurement, ensure_ascii=False, indent=2)[:1450], fill=(210, 220, 200), font=b11.F_SMALL)
    return canvas


def _runtime_icon_layer_for_frame(icon: Image.Image) -> Image.Image:
    layer = Image.new("RGBA", FRAME_SIZE, (0, 0, 0, 0))
    cx, cy = _badge_center()
    layer.alpha_composite(icon, (round(cx - icon.width / 2), round(cy - icon.height / 2)))
    return layer


def frame_with_runtime_icon(frame: Image.Image, state: str, runtime_icons: dict[str, Image.Image]) -> Image.Image:
    out = frame.copy().convert("RGBA")
    out.alpha_composite(_runtime_icon_layer_for_frame(runtime_icons[state]))
    return out


def make_fit_qa(frames: list[Image.Image], runtime_icons: dict[str, Image.Image], measurement: dict) -> Image.Image:
    ...
    draw = ImageDraw.Draw(canvas)
    draw.text((42, 24), "B2.6 v0.9.12 16-point visual QA: clean atlas badge + runtime icon", fill=(242, 238, 210), font=b11.F_HEAD)
    draw.text((42, 64), "Each state has four 200% checkpoints; badge crop includes the runtime icon layer. Manifest records evidence_ready, not final visual PASS.", fill=(205, 224, 198), font=b11.F_NOTE)
    for idx, state in enumerate(STATE_ORDER):
        row_y = 126 + idx * 350
        bg = Image.new("RGBA", FRAME_SIZE, (7, 19, 21, 255))
        bg.alpha_composite(frame_with_runtime_icon(frames[idx], state, runtime_icons))
        gx, gy = globe["center"]
        gr = globe["radius"]
...
def _draw_runtime_icons_on_canvas(canvas: Image.Image, runtime_icons: dict[str, Image.Image]) -> None:
    scale = 1.5
    asset_to_runtime = scale / 2.0
    positions = [(66, 36), (66, 294), (66, 552), (66, 810)]
    cx, cy = _badge_center()
    for idx, state in enumerate(STATE_ORDER):
        icon = runtime_icons[state]
        display_size = (max(1, round(icon.width * asset_to_runtime)), max(1, round(icon.height * asset_to_runtime)))
        display_icon = icon.resize(display_size, Image.Resampling.LANCZOS)
        card_x, card_y = positions[idx]
        x = round(card_x + cx * asset_to_runtime - display_icon.width / 2)
        y = round(card_y + cy * asset_to_runtime - display_icon.height / 2)
        canvas.alpha_composite(display_icon, (x, y))


def _draw_runtime_qa_slots(canvas: Image.Image) -> None:
    draw = ImageDraw.Draw(canvas)
    scale = 1.5
    positions = [(66, 36), (66, 294), (66, 552), (66, 810)]
    ...
    for x, y in positions:
        ...
        draw.rectangle((x, y, x + round(EXPORT_SIZE[0] * scale), y + round(EXPORT_SIZE[1] * scale)), outline=(101, 255, 138), width=2)


def make_runtime_preview(frames: list[Image.Image], runtime_icons: dict[str, Image.Image], show_qa: bool) -> Image.Image:
    canvas = b11.make_runtime_preview(frames, b11.load_contract(), False).convert("RGBA")
    _draw_runtime_icons_on_canvas(canvas, runtime_icons)
    if show_qa:
        _draw_runtime_qa_slots(canvas)
    draw = ImageDraw.Draw(canvas)
    draw.rectangle((448, 30, 1400, 112), fill=(7, 19, 21))
    draw.text((450, 34), "Python v0.9.12 B2.6 left_region_card runtime fill", fill=(243, 239, 214), font=b11.F_HEAD)
    draw.text((450, 76), "Atlas badge base is clean; state icons are runtime PNG ingredients in the action_badge slot.", fill=(217, 222, 199), font=b11.F_NOTE)
...
def aggregate_gates(per_state: list[dict], measurement: dict, manual_pass: bool, godot_pass: bool) -> dict:
    ...
    return {
        ...
        "badge_state_color_consistency": {
            ...
            "basis": "badge inner core belongs to frame-color family; atlas contains zero baked state glyph pixels",
            "per_state": {
                item["state"]: item["gates"]["badge_state_color_consistency"]
                for item in per_state
            },
        },
        "ingredient_purity": measurement["ingredient_purity"],
        ...
        "manual_16point_visual_check": {
            ...
            "basis": "inspect 495: four states x four checkpoints (globe arc/right edge/bottom edge/badge with runtime icon); execution output is evidence-ready only, reviewer/user owns PASS/FAIL",
            "evidence": str(OUT_FIT_QA),
            "checks": manual_checks,
        },
        "derived_art_quality_check": {
            "status": "evidence_ready",
            "basis": "inspect 489 and 490: derived status colors and frozen ingredient purity evidence are prepared for reviewer; execution self-check is not final PASS",
...
def make_manifest(measurement: dict, per_state: list[dict], geometry_metrics: list[dict], checks: dict, manual_pass: bool, godot_pass: bool) -> dict:
    ...
    return {
        ...
        "round": ROUND_ID,
        "date": "2026-07-09",
        "status": "b2_6_runtime_badge_evidence_ready_pending_review" if evidence_ready else "b2_6_runtime_badge_pending_full_chain",
        "contract": str(b11.CONTRACT_PATH),
        "contract_frozen_fields_changed": False,
        "pipeline": "available single master -> edge-expanded full-window cutout -> clean badge base -> pre-recolor old-content scan -> derived state skins with badge-core recolor only -> rectangular photo underlay -> independent globe patch -> runtime text + runtime badge icon",
        "inputs": {
            ...
            "previous_b2_5_manifest": str(BASE / "486-world-map-wmw-v0-9-11-left-card-b2-5-manifest.json"),
        },
        "outputs": {
            ...
            "runtime_badge_icons": {
                state: str(GODOT_INGREDIENT_DIR / f"left_region_card_b26_runtime_icon_{state}.png")
                for state in STATE_ORDER
            },
            "globe_patch_ingredient": str(GODOT_INGREDIENT_DIR / "left_region_card_b26_independent_globe_patch.png"),
        },
        "runtime_tokens": {
        ...
        "prohibitions_observed": {
            ...
            "badge_glyph_baked_into_atlas": False,
            "old_content_leftover_scan_after_recolor": False,
            "unfrozen_ingredient_used_for_composite": False,
            "contract_frozen_fields_changed": False,
            "other_classes_batch_produced": False,
            "other_layers_modified": "badge state glyphs moved to runtime icon layer; B2.4 globe structure, edge scan, and photo layer logic retained",
        },
    }
...
def write_outputs(manual_pass: bool = False) -> dict:
    source_frames = b22.load_atlas_frames(B_ATLAS)
    frames, shells, master_shell, mask, globe_patch, runtime_icons, measurement, per_state, geometry_metrics = compose_single_master_frames()
    OUT_COMPARE.parent.mkdir(parents=True, exist_ok=True)
    make_compare_board(source_frames, frames, measurement).save(OUT_COMPARE)
    make_ingredient_board(shells, master_shell, mask, globe_patch, runtime_icons, measurement).save(OUT_INGREDIENTS)
    ...
    make_runtime_preview(frames, runtime_icons, False).save(OUT_RUNTIME)
    make_runtime_preview(frames, runtime_icons, True).save(OUT_RUNTIME_QA)
    make_fit_qa(frames, runtime_icons, measurement).save(OUT_FIT_QA)
    ...
    globe_patch.save(GODOT_INGREDIENT_DIR / "left_region_card_b26_independent_globe_patch.png")
    for state, icon in runtime_icons.items():
        icon.save(GODOT_INGREDIENT_DIR / f"left_region_card_b26_runtime_icon_{state}.png")
    for state, shell in zip(STATE_ORDER, shells):
        shell.save(GODOT_INGREDIENT_DIR / f"left_region_card_b26_runtime_badge_hollow_shell_{state}.png")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Build WMW left card B2.6 runtime-badge single-master atlas.")
    parser.add_argument(
        "--manual-ok",
        action="store_true",
        help="Deprecated compatibility flag; B2.6 only marks visual checks evidence_ready pending reviewer/user judgment.",
    )
