...
$captureScript = 'res://tests/capture_world_map_wmw_right_dossier_runtime_v092.gd'
...
Write-Host 'Running WMW v0.9.2 right-dossier candidate A2 capture (windowed opengl3)...'
& $godot --path $projectPath --resolution 1920x1080 --windowed --audio-driver Dummy --rendering-driver opengl3 -s $captureScript
if ($LASTEXITCODE -ne 0) {
    throw 'WMW v0.9.2 right-dossier candidate A2 Godot capture failed.'
}

$expected = @(
    '601-world-map-wmw-v0-9-2-right-dossier-candidate-a2-godot-runtime.png',
    '602-world-map-wmw-v0-9-2-right-dossier-candidate-a2-godot-runtime-qa.png'
)
...
Write-Host 'WMW v0.9.2 right-dossier candidate A2 Godot capture passed.'
