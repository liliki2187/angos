...
$captureScript = 'res://tests/capture_world_map_wmw_right_dossier_runtime_v090.gd'
...
Write-Host 'Running WMW v0.9.0 right-dossier capture (windowed opengl3)...'
& $godot --path $projectPath --resolution 1920x1080 --windowed --audio-driver Dummy --rendering-driver opengl3 -s $captureScript
if ($LASTEXITCODE -ne 0) {
    throw 'WMW v0.9.0 right-dossier Godot capture failed.'
}

$expected = @(
    '585-world-map-wmw-v0-9-0-right-dossier-candidate-a-godot-runtime.png',
    '586-world-map-wmw-v0-9-0-right-dossier-candidate-a-godot-runtime-qa.png'
)
...
Write-Host 'WMW v0.9.0 right-dossier Godot capture passed.'
