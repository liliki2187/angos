...
$exportScript = 'res://tests/export_world_map_wmw_right_dossier_runtime_fixture_v094.gd'
$fixturePath = Join-Path $repoRoot 'gd_project\Assets\ui\angus_packaging\world_map\wmw_v094_right_dossier_candidate_a4\right_dossier_candidate_a4_runtime_fixture.json'
...
Write-Host 'Exporting WMW v0.9.4 right-dossier production runtime fixture (logic-only headless)...'
& $godot --headless --path $projectPath -s $exportScript
if ($LASTEXITCODE -ne 0) {
    throw 'WMW v0.9.4 right-dossier semantic fixture export failed.'
}
...
Write-Host 'WMW v0.9.4 right-dossier production runtime fixture passed.'
