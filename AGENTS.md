...
- **Git 云端提交固定工作流**：用户要求盘点、暂存、提交或推送当前工作区改动时，必须读取并执行 [`skills/git-cloud-submit/SKILL.md`](./skills/git-cloud-submit/SKILL.md)。模型顺序固定为 Sol（最高能力）> Terra（默认协调）> Luna（最快、最低成本）；完整分工、升级条件、授权边界和运行时回退只在该技能维护，不在顶层重复。
