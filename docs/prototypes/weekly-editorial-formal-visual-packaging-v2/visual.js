(() => {
  ...
  function imageFormatFor(node) {
    return node.classList.contains("main-slot")
  ...
  function decorate() {
    ...
    document.querySelectorAll(".slot:not(.is-occupied)").forEach((slot) => {
      ...
      slot.dataset.imageFormat = slot.classList.contains("main-slot")
        ? "headliner_landscape_15_8"
        : "square";
  ...
  document.title = "发刊编辑界面｜正式视觉包装候选 v2";
  const stage = document.querySelector(".stage");
  if (stage) stage.dataset.artifact = "formal_visual_packaging_candidate_v2";
