...
> **最后更新**：2026-07-17（A232：用户追问中央红线 / 深链双条用途并同意试删或重想。复核确认两者重复左下推进后果与右侧任务情报，常驻双条退出默认宿主；地图扩为 `[426,96,924,936]`。另提供 `[450,942,876,90]` 条件式“最近世界变化”结构态，但当前 `world_receipts` 没有事件 ID 与 before/after，不能接入。v4 两张 1920×1080 黑白结构态与审计已生成，等待用户裁决；A229 左右职责闭合继续有效，尚未改合同、Godot 或有色生图。）
...
| 2 | 页面结构 / 职责 | **正确整屏宿主已落地** | 独立 assembly 按 30 的三区职责、575 的视觉语言接线；旧宿主与禁止元素未进入运行态 |
...
| 6 | 无字 clean-sprite brief | **右 dossier 已落地首张候选** | 576/577 brief 已由 579-582 三个真实生图源与无字 parent / secondary / primary 母版验证；visible body / shadow bbox 独立量测 |
| 7 | 生图 / 素材生产 | **右 dossier A5.1 原位状态切片完成；抽屉 skin 已退役** | 零 imagegen、零新 raster 配料；复用 A5 parent / carrier、A1 照片与 A3 橄榄 CTA，照片仍为精确 `69:44` 图例 |
...
| 10 | 多状态截图验收 | **结构与状态通过；地图美术待生产** | collapsed / expanded / locked / QA / GIF 齐备；双 agent P0=0/P1=0，`map_panel=structure_only` 不升格为视觉通过 |
