...
- 当前正式 UI 的设计与迭代基线统一按 **1920x1080** 视口处理；项目设置中的默认设计分辨率也应保持与此一致。
- 正式 UI 优先使用 `.tscn` 场景和可复用组件场景完成布局、层级和基础控件结构。
- UI 脚本默认只负责节点引用、信号连接、数据绑定、显隐切换和状态刷新，不负责手工拼装固定布局。
- 除浮动模态框、特效覆盖层和少量受控装饰节点外，正式 UI 应优先使用 `Container`、锚点、`size_flags`、边距和滚动容器做相对布局，而不是依赖大块绝对坐标和固定像素定位。
- 正式整页 UI 不应依赖 `HSplitContainer.split_offset`、硬编码大偏移或子控件最小尺寸失控来“顶开”版面；如果内容数量可能增长，优先使用 `ScrollContainer` 或其他可裁剪容器封住可变区域。
- 对于数量可变的列表、卡片、日志、槽位等内容，优先实例化预制的 item scene，而不是直接 `new` 原始 `Control` / `Button` / `Label` / `Container` 树。
- UI 不拥有玩法真状态，只读取状态并通过信号或明确接口发出动作请求。
- 新的正式整页 UI 在提交前，至少要确认 `1920x1080` 基线下没有裁切，并在一次缩放或全屏场景下不会因为绝对定位导致大面积返工。
...
- 不要在正式整页 UI 中把主要区域写成依赖固定像素坐标的绝对布局，除非它本身就是浮动模态框或受控特效层。
