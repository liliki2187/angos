...
<div class="nm-tip" style="margin-top:6px;font-size:11px;padding:6px;">拖到中间头版版位</div>`;
    ...
    await showWeek1SoftTutorialModal(
      ...
      [
        `${tutorialSoftFigure(
          ...
          "合成台示意：左侧素材拖入右侧卡槽",
          "示意：左为素材区；右为各定型报道卡槽（红/金）。",
        )}<div class="tutorial-soft-sheet">
          <h4 class="tutorial-soft-h4">合成台在做什么？</h4>
          <p class="tutorial-soft-lead">左侧是您带回的<strong>现象、认知、情报、工具</strong>；右侧切换<strong>抢先快讯、深度报道、专栏、爆炸性新闻</strong>等定型。不同<strong>情报牌领域</strong>（科学侧/秘闻/流行）会影响成稿标签与走向。</p>
        </div>`,
        `<div class="tutorial-soft-sheet">
          <h4 class="tutorial-soft-h4">槽位颜色</h4>
          <ul class="tutorial-soft-ul">
            <li>按<strong>红色</strong>槽放入必需素材；<strong>金色虚线</strong>槽可放工具，略抬成功率。</li>
            <li>页面下方有<strong>报道类型与合成方式一览</strong>，可随时对照各类型所需卡牌。</li>
          </ul>
        </div>`,
        `<div class="tutorial-soft-sheet">
    ...
    scheduleWeek1SoftTutorial(
      ...
      [
        `${tutorialSoftFigure(
          ...
          <ul class="tutorial-soft-ul">
            <li>点选队员卡片，最多 <strong>5</strong> 人。</li>
            <li>上方概率是本次外拍的参考。</li>
  ...
  const INTEL_DOMAIN_ZH = { sci: "科学侧", occult: "秘闻", pop: "流行" };
  ...
  /** 假设所有空「必要」槽均填入正确类型时，当前配方是否仍可完成 */
  function synthAllRequiredEmptiesCompletable() {
    ...
    for (const t of layout) {
      ...
      if (slots[t.key]) continue;
      ghosts.push({ kind: t.kind });
    }
    return validateRecipe(recipe, [...cards, ...ghosts]);
  }

  /** 仍可走向合法合成的空「必要」槽 key（与幽灵牌判定一致） */
  function getSynthValidEmptyRequiredSlotKeys() {
    const set = new Set();
    if (!synthAllRequiredEmptiesCompletable()) return set;
    const layout = synthSlotLayout(state.synthRecipe);
    const slots = state.synthCraftSlots;
    for (const t of layout) {
      if (t.required && !slots[t.key]) set.add(t.key);
    }
    return set;
  }

  /** 必要素材已齐、可选工具槽空时，提示可加工具 */
  function synthToolSlotSoftHintEligible() {
    const recipe = state.synthRecipe;
    const layout = synthSlotLayout(recipe);
    const toolDef = layout.find((s) => s.kind === "tool" && !s.required);
    if (!toolDef || state.synthCraftSlots[toolDef.key]) return false;
    return validateRecipe(recipe, selectedSynthCards());
  }

  /** 将 dragKind 放入空槽 slotKey 后，其余空必要槽用幽灵补全，是否可满足配方 */
  function synthCompletableIfDropOnSlot(slotKey, dragKind) {
    const recipe = state.synthRecipe;
    const layout = synthSlotLayout(recipe);
    const slots = state.synthCraftSlots;
    const def = layout.find((s) => s.key === slotKey);
    if (!def || def.kind !== dragKind || slots[slotKey]) return false;
    const cards = selectedSynthCards();
    const dropped = [{ kind: dragKind }];
    const ghosts = [];
    for (const t of layout) {
      if (!t.required) continue;
      if (t.key === slotKey) continue;
      if (slots[t.key]) continue;
      ghosts.push({ kind: t.kind });
    }
    return validateRecipe(recipe, [...cards, ...dropped, ...ghosts]);
  }

  /** 拖拽/预览某类型时，空槽是否显示 ×（类型不符或放入后仍无法完成配方） */
  function getSynthRejectSlotKeysForDragKind(dragKind) {
    const reject = new Set();
    if (!dragKind) return reject;
    const layout = synthSlotLayout(state.synthRecipe);
    const slots = state.synthCraftSlots;
    for (const s of layout) {
      ...
      if (!synthCompletableIfDropOnSlot(s.key, dragKind)) reject.add(s.key);
    }
    return reject;
  }

  function resetSynthPointerHints() {
    state.synthDragKind = null;
    state.synthHintCardId = null;
  }

  function currentSynthPreviewKind() {
    if (state.synthDragKind) return state.synthDragKind;
    if (state.synthHintCardId) {
      const c = getCardById(state.synthHintCardId);
      return c ? c.kind : null;
    }
    return null;
  }

  function applySynthWorkbenchHintClasses() {
    const root = el.synWorkbench;
    if (!root) return;
    const slots = state.synthCraftSlots || {};
    const validReq = getSynthValidEmptyRequiredSlotKeys();
    const toolSoft = synthToolSlotSoftHintEligible();
    const layout = synthSlotLayout(state.synthRecipe);
    const toolDef = layout.find((s) => s.kind === "tool" && !s.required);
    const previewKind = currentSynthPreviewKind();
    const rejectKeys = previewKind ? getSynthRejectSlotKeysForDragKind(previewKind) : new Set();

    root.querySelectorAll(".syn-drop-slot").forEach((zone) => {
      const key = zone.getAttribute("data-slot-key");
      zone.classList.remove("syn-drop-hint-valid", "syn-drop-hint-tool", "syn-drop-hint-reject");
      if (!key) return;
      const id = slots[key];
      const def = layout.find((s) => s.key === key);
      if (!def) return;
      const rejectHere = Boolean(previewKind && !id && rejectKeys.has(key));
      if (rejectHere) {
        zone.classList.add("syn-drop-hint-reject");
        return;
      }
      if (!id && def.required && validReq.has(key)) zone.classList.add("syn-drop-hint-valid");
      if (toolDef && key === toolDef.key && !id && toolSoft) zone.classList.add("syn-drop-hint-tool");
    });
  }
  ...
  function buildSynthPreview(recipe, cards) {
    if (!cards.length) {
      return "尚未放入素材。请将左侧卡牌拖入右侧红色必要槽；虚线槽为可选工具位。爆炸性新闻需另拖入「爆料卡」（仅特殊途径获得，不可合成）。\n提示：情报牌随探索领域（科学侧/秘闻/流行）不同，会影响成稿标签与数值走向；可搭配不同定型调整侧重。";
    }
  ...
  function renderSynthInventory() {
          ...
          c.kind === "tool"
            ...
            : c.kind === "cognition"
              ...
              : c.kind === "tipoff"
                ...
                : c.kind === "intel"
                  ? `<span class="syn-badge">${INTEL_DOMAIN_ZH[c.sourceType] || c.sourceType || "情报"}</span><span class="syn-badge">证据 ${c.evidenceValue || 0}</span>`
                  : `<span class="syn-badge">证据 ${c.evidenceValue || 0}</span>`;
        const hintSel = state.synthHintCardId === c.id ? " syn-item-hint-selected" : "";
        return `<div class="syn-item syn-item-draggable${placed ? " syn-item-slotted" : ""}${hintSel}" draggable="true" data-card="${c.id}" data-kind="${c.kind}" title="拖到右侧卡槽；点击可预览槽位提示">
    ...
    el.synInventory.querySelectorAll(".syn-item-draggable").forEach((node) => {
      node.addEventListener("dragstart", (ev) => {
        ...
        state.synthHintCardId = null;
        state.synthDragKind = kind || null;
        applySynthWorkbenchHintClasses();
      });
      ...
      node.addEventListener("click", (ev) => {
        ev.stopPropagation();
        const id = node.getAttribute("data-card");
        state.synthHintCardId = state.synthHintCardId === id ? null : id;
        applySynthWorkbenchHintClasses();
  ...
  function renderSynthesis() {
    ...
    if (el.synthesisHint) {
      el.synthesisHint.textContent = `配方说明：${recipeLabel(state.synthRecipe)}。现象与认知可反复用于公开报道；情报为材料每次合成会消耗。不同领域的情报（科学侧/秘闻/流行）会拉动标签与数值走向。爆料与工具按原规则。同题材本周多篇公开稿上版时报纸结算基础值递减。`;
    }
    const craftBtn = document.getElementById("craftBtn");
    if (craftBtn) {
      craftBtn.disabled = !validateRecipe(state.synthRecipe, cards);
      craftBtn.textContent = "执行合成";
    }
    const recipeIds = ["recipe1", "recipe2", "recipe3", "recipe4"];
    const recipeKeys = ["r1", "r2", "r3", "r4"];
    recipeIds.forEach((bid, i) => {
      const b = document.getElementById(bid);
      if (b) b.classList.toggle("primary", state.synthRecipe === recipeKeys[i]);
    });
    renderSynthReports();
  }
  ...
  function enterSynthesisPhase() {
    ...
    document.getElementById("synthesisSub").textContent = `第 ${state.week} 周 · 已带入探索线索 ${state.pendingClues.length} 条。请按定型配方合成报道；换用不同领域的情报可调整成稿走向，再进入组版。`;
  ...
  let synthHintOutsideClickBound = false;

  function bindSynthesisUi() {
    if (!synthHintOutsideClickBound) {
      synthHintOutsideClickBound = true;
      document.addEventListener("click", (ev) => {
        if (state.phase !== "synthesis") return;
        const t = ev.target;
        if (t.closest && t.closest(".syn-item-draggable")) return;
        if (t.closest && t.closest(".syn-drop-slot")) return;
        if (!state.synthHintCardId) return;
        state.synthHintCardId = null;
        applySynthWorkbenchHintClasses();
        if (el.synInventory) renderSynthInventory();
      });
    }
