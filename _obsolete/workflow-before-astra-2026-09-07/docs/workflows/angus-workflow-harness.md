...
开工第一条进度更新还必须用自然语言说明“用户明示需求 / 为闭环必须补完的隐含功能 / 可以延期的生产化工作 / 推荐本轮做到哪里”。完整规则见 §2.1；这段说明不是要求用户每次审批，也不得扩成一张重表。
...
### 2.1 前置范围说明与两阶段可见迭代（2026-07-15 起）

来源：A214 同纸色双版与局部换稿本身不复杂，但父级没有在开工前说明隐含功能和生产化成本，直接把连续看图修订扩成完整生产候选，导致首次可见结果超过两小时。用户确认大部分补全功能本身必要，真正的问题是**没有提前告知、没有裁决是否同批完成、每轮都做全量验证和生成**。

本节是 Level 1 的人类可读执行规则，不新增机器 hard gate，因此不修改 `workflow-gates.yml`。

#### A. 开工第一条更新：四项范围预告

normal 以上任务，父级在任务开始前；若无法提前判断，则在第一次只读检查后，必须用 3–6 行自然语言告诉用户：

1. **明示需求**：用户直接要求改变什么。
2. **必要补全**：如果不一起做会导致功能断裂、数据错误、没有退出路径或验收失真的隐含工作。
3. **可延期工作**：完整状态矩阵、替代输入路径、全量 QA、资产升版、全套文档同步等不阻塞第一次看效果的工作。
4. **本轮建议**：哪些同批完成，哪些在第一个可见检查点通过后再做；给出首次可见结果的大致时间级别。

不要把该说明写成 Router / gate 术语堆，也不要默认停下来等用户逐项批准。只有以下任一情况出现时，才需要明确交给用户裁决后再扩大：

- 新增用户没有描述的玩法或交互语义；
- 预计让首次可见结果额外延后 30 分钟以上；
- 需要重生 / 替换一组正式资产，而不是复用当前候选；
- 需要把预览直接升格为 manifest、atlas、生产真源或批量资源；
- 存在两种后果明显不同、无法从现有规则判断的方案。
...
- 目标是尽快回答用户当前在看的那个问题。
- UI / 美术连续纠偏默认先交 `runtime_state_preview` 或等价局部候选，不自动升成 `production_candidate`。
- 只做使当前操作闭环的必要功能；不因为最终会需要，就把所有替代路径、全状态覆盖和资产治理一起挡在第一次截图前。
- 桌面 UI 首次可见检查点原则上控制在 30–45 分钟；若判断会超过 40 分钟，必须在继续前说明新增耗时项与原因。

**阶段二：生产升格（production hardening）**

- 只有用户确认方向，或用户一开始就明确要求正式落地 / 生产候选时进入。
- 再补完整边界状态、全量回归、正式生图、sidecar、manifest、合同升版、完整 QA 与长期文档同步。
- 阶段一已经验证且本轮未失效的证据直接复用，不得为“流程完整”机械重跑。

正确性保护不允许延期：会导致稿件丢失、重复分配、确认快照与真实状态分叉、不可逆误操作或核心流程卡死的事务与锁定逻辑，必须与首次可运行版本同批完成。可以延期的是替代输入方式、低概率边界、视觉精修和生产治理，不是数据安全。

#### C. 增量验证矩阵

| 改动类型 | 可见检查点最低验证 | 生产升格追加验证 |
| --- | --- | --- |
| 纯视觉 / 布局 | 受影响状态真实截图；Godot 改动跑基础 smoke | 长文案、全状态、几何 / 色彩定量 QA、整屏回归 |
| 交互状态 | 直接相关主路径 6–12 项断言；短动态证据；基础 smoke | 完整状态矩阵、取消 / 非法输入 / 冻结、全量交互审计 |
| 单一资产 | 尺寸 / 安全区 / 运行时落位；只重生被修改资产 | provenance、hash / DeltaE / 全状态组合、manifest 与合同升版 |
| 文档 | 链接、术语与受影响真源对照 | 只有实现也改变时才跑 Godot / 截图，不因文档整理机械跑全链 |

增量验证遵守失效传播：

- 修改了什么，就验证它和直接消费者；没有被修改且依赖未变化的合同、截图和 QA 引用最近一次通过证据。
- 可见改动仍按项目硬规则提供真实截图；动态改动仍提供真实动图 / 视频，但中间版允许短链路，不要求每次重做完整演示片。
- 生图只针对被修改的资产 class；不因一个动态边线或按钮逻辑变化重生无关纸壳、报道图或 atlas。
- 全量回归只在生产升格、跨系统改动、真源冻结、发布前或增量测试发现外溢时触发。

#### D. A214 对照

A214 若按本规则执行：

- 明示需求：双版同纸色、替换按钮局部揭示、拖拽去留。
- 必要补全：候选投放、版位退回候选、唯一分配、确认冻结。
- 应提前说明并裁决是否同批：跨版原子交换、两套点击顺序、精细取消优先级。
- 第一次截图前只跑核心交互断言、Godot smoke 和三态短动态；用户确认后再做 42 项审计、真实副头版 v2、DeltaE、manifest 与合同全量升版。
...
- 截图 / 测试 / 审查对象与目标阶段一致。
- 关键局部已裁切或等价检查。
- 局部视觉修正 / image edit 若要求保留既有构图，必须执行 trial `scope_invariant` gate：生成前声明可变 ROI 与不可变项，生成后做全图前后对照；整图重采样改变 ROI 外轮廓时不得声称“局部修正通过”。
- 至少写出一个反向读法，并检查是否仍成立。
- 文档 / 实现 drift 已判断。
- 用户采纳项是否需要沉淀已判断。
...
- 进入 `filled_state_text_mock` 前，先交付一张与目标分辨率一致的黑白功能结构稿，并逐项核对已采纳决策、真实功能清单与尚未实现的运行命令；缺一项就不得靠风格生图补齐。
- 黑白结构稿还必须做“信息价值 / 容器体量”审计：只读短状态不得占用接近主决策卡的完整大纸；`NO_TEXT / NO_HIT` 收口不得因前景面积和粗边框读成第二功能面板。已有 frozen 类不适合当前职责时，应新增密度变体，不得直接拉扁或因“形式已确定”而原样误用。
- 删除过大容器后必须再次做“职责闭合”审计：若同等面积只是变成栏内无框背景、呼吸区或 `NO TEXT / NO HIT`，仍判失败。桌面有界栏位的底部应由该栏真实职责、固定动作或正常安全边距收口；不得用移除边框冒充解决空白。
- 通过体量审计后还必须做“独立玩家问题”审计：每个常驻状态组件必须能说出自己独占回答的问题。若同一字段已经由日程后果、对象详情或任务列表表达，不能仅因已有组件外形或需要填满底部而再做一份常驻回执；条件式变化反馈必须来自真实 before/after 差量，当前值拼句不得冒充事件。
...
- `screen_component_completion_integrity`：任何“整屏 filled-state / 完整视觉目标”声明前，必须列出本屏全部必需可见组件类，并逐类登记本轮产物类型、来源与完成证据；沿用旧参考图像素的区域只能标成 `reference-only`，单栏或单组件回嵌不得冒充整屏完成。
